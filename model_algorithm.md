...
- 전체 코호트 기준 민감도가 1차 단독 베이스라인 대비 5%p 이상 떨어지지 않을 것
  (sentinel 후보 th2=-inf는 항상 1차 결과를 그대로 재현하므로 탐색 공간에 항상
  유효한 후보로 포함된다).

최종 판정은 PPV가 아니라 **전체 코호트의 Sensitivity/Specificity delta**로 내린다
(PPV는 임계값 선택을 위한 내부 목적함수일 뿐).
...
| 현재까지 최선 결과 | logreg + PCA(k=4) + stage1 score: internal spec_delta=+8.6%, external +5.4% | HGB + cluster_band(8) + stage1 score: internal spec_delta=+10.9%, external +8.0% (현재 최선) |
| 출력 위치 | `outputs/stage2_aec_residual_reclassify/` | `outputs/stage2_aec_residual_reclassify_bandfeat/` |

두 파일 모두 model-selection sweep(모델 종류 x curve feature x stage-1 score 포함 여부)을
내부 OOF로만 수행하고, 외부 코호트는 최종 선택된 설정에 대해 단 한 번만 적용한다
(`feedback_new_scripts_under_code_root` 참고 — 새 스크립트는 `code/` 바로 아래에 위치).
