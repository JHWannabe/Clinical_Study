...
## 8. 비교 실험: AEC 입력 표현 및 PCA 성분 수

`1_aec_residual_reclassify.py`의 함수를 그대로 재사용해(파이프라인 로직 재구현 없음) 이
파이프라인이 내린 두 가지 설계 선택 — ① 4.2절의 BMI 잔차화, ② 4.2절의 자동 PCA 성분 수 —
이 실제로 도움이 되는지 검증하는 두 스크립트를 추가했다.

### 8.1 `1_1_aec_input_variant_comparison.py` — AEC 입력 표현 비교

두 축을 비교한다:

- **축 1 (AEC 표현 방식)**: 같은 임상 Stage-1+Stage-2 구조에 AEC 입력만 바꿔서 비교
  - *patient-normalized*: 환자 평균으로만 나눈 곡선 (4.2절의 잔차화 단계 없이, 정규화만)
  - *BMI-residualized*: 현재 파이프라인이 쓰는, 임상 변수 회귀 잔차 곡선(4.2절 그대로)
- **축 2 (AEC 단독 vs 결합)**: 임상 변수를 전혀 쓰지 않고 AEC PCA 점수만으로 단일 단계
  로지스틱 회귀(`clinic-only_baseline.py`와 동일한 OOF/목표민감도 임계값 방식)를 학습해,
  Clinical-only baseline 및 위 두 결합 모델과 비교

Stage-1 게이트, Stage-2 sweep/모델선택 로직은 두 표현 방식 모두 동일하게 적용해, 차이가 오직
AEC 입력 표현에서만 나도록 했다.

**결과** (`outputs/1_1_aec_input_variant_comparison/aec_input_variant_comparison.csv`):

| model | sens (int) | spec (int) | AUC (int) | sens (ext) | spec (ext) | AUC (ext) |
| --- | --- | --- | --- | --- | --- | --- |
| Clinical only (stage-1) | 0.907 | 0.536 | 0.829 | 0.929 | 0.490 | 0.834 |
| AEC only — patient-normalized | 0.907 | 0.143 | 0.636 | 0.922 | 0.093 | 0.568 |
| AEC only — BMI-residualized | 0.907 | 0.074 | 0.521 | 0.943 | 0.036 | 0.493 |
| Clinical + AEC — patient-normalized | 0.891 | 0.569 | — | 0.915 | 0.517 | — |
| Clinical + AEC — BMI-residualized | 0.899 | 0.579 | — | 0.922 | 0.508 | — |

("Clinical + AEC — BMI-residualized" 행은 현재 파이프라인 자체와 같은 AEC 입력을 쓰므로,
`outputs/1_aec_residual_reclassify/stage1_vs_stage2_summary.csv`의 internal spec_combined
값과 정확히 일치한다 — 두 스크립트가 같은 코드 경로를 탄다는 교차 검증이기도 하다.)

해석:

- **AEC 단독은 임상 변수 없이는 쓸모가 없다**: 두 표현 방식 모두 AEC-only 모델의 AUC가
  external에서 0.5 근처(patient-normalized 0.568, BMI-residualized 0.493 — 동전 던지기
  수준)로 떨어진다. 민감도가 높아 보이는 건(0.92~0.94) 목표민감도 90%에 맞춰 임계값을
  낮게 잡았기 때문일 뿐, 특이도가 0.04~0.09로 사실상 전원을 양성 판정하는 것과 다르지
  않다. AEC-128 곡선은 임상 변수와 결합됐을 때만 의미 있는 재분류 신호가 된다.
- **BMI 잔차화는 patient-normalized보다 나은 방향이지만 폭이 크지 않다**: 결합 모델
  기준 internal spec은 BMI-residualized(0.579)가 patient-normalized(0.569)보다 근소하게
  높고, external은 반대로 patient-normalized(0.517)가 BMI-residualized(0.508)보다
  근소하게 높다 — 4.2절이 잔차화의 근거로 든 Simpson's paradox식 BMI 교란 제거가 방향은
  맞지만, 이 표본 규모에서 그 효과가 두 코호트 모두에서 일관되게 이기는 정도는 아니다.

### 8.2 `1_2_aec_pca_k_comparison.py` — PCA 성분 수(k) 비교

현재 파이프라인(4.2절)은 PCA 성분 수를 누적 설명분산 90%/95% 도달 시점으로 **자동** 결정한다.
이 스크립트는 그 자동 선택 대신 k를 4, 6으로 **고정**했을 때 결과가 어떻게 달라지는지
비교한다 — 표준화·잔차화·Stage-2 sweep(model_type x use_stage1_score)·frozen-external
평가는 모두 동일하게 재사용.

**결과** (`outputs/1_2_aec_pca_k_comparison/pca_k_comparison.csv`):

| model | k | 설명분산(내부) | spec_delta (int) | spec_delta (ext) | 판정 |
| --- | --- | --- | --- | --- | --- |
| Clinical only (stage-1) | — | — | 0.000 (기준) | 0.000 (기준) | PASS |
| Clinical + AEC (PCA k=4) | 4 | 0.917 | +0.030 | +0.011 | PASS |
| Clinical + AEC (PCA k=6) | 6 | 0.963 | +0.051 | +0.022 | PASS |
| 현재 파이프라인(자동 k, `1_aec_residual_reclassify.py`) | 가변 | — | +0.043 | +0.018 | PASS |

k=4는 internal·external 모두 현재 파이프라인(자동 k)보다 낮고, k=6은 internal(+0.051 vs
+0.043)·external(+0.022 vs +0.018) 모두 현재 파이프라인을 근소하게 앞선다 — 즉 이번 실행
기준으로는 "성분을 더 많이 쓰는 쪽"이 자동 선택보다 살짝 낫다. 다만 두 코호트 모두 그 격차가
1~2%p 수준이고, 6장의 표본 크기 경고(`residual_reclassify_cnn_algorithm.md` 및 위
`cnn_variant_comparison.md` 4절)와 마찬가지로 한 번의 split·한 번의 실행만으로 "k=6이
확실히 더 낫다"고 단정할 근거는 아니다.
