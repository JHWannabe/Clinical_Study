...
1. 표준화된 clinical 피처(age/height/weight/sex)에 대해 각 slice의 patient-normalized AEC 값을 내부 코호트에서만 선형회귀로 fit한다.
...
| 구분 | `code/1_aec_residual_reclassify.py` | `code/2_aec_residual_reclassify_bandfeat.py` |
| --- | --- | --- |
| Curve feature | 전역 functional PCA만 (분산 90%/95% 목표, k=4~6) | PCA + `band`(등간격 4/8/16/32구간 평균) + `cluster_band`(data-driven 구간 평균) + `combo`(band+PCA 결합) + `radiomics1d`(1D 신호로 변형한 first-order + GLCM + GLRLM 텍스처 피처, n_bins=8) |
...
| 출력 위치 | `outputs/aec_residual_reclassify/` | `outputs/aec_residual_reclassify_bandfeat/` |
