...
`code/3_aec_cnn_reclassify.py`의 구현 설명. 설계 원칙은
...
- **Stage 2** (`code/3_aec_cnn_reclassify.py`): Stage 1에서 "양성(screen positive)"으로 분류된 환자군만을 대상으로, AEC-128 곡선에서 임상 변수로 설명되지 않는 잔차(residual) 정보를 1D-CNN으로 학습해 특이도(Specificity)를 끌어올리는 재분류 모델.
...
| --- | --- | --- | --- |
| PCA (`1_aec_residual_reclassify.py`) | +0.043 | +0.018 | PASS / PASS |
| band/cluster_band HGB (`2_aec_residual_reclassify_bandfeat.py`) | +0.049 | +0.025 | PASS / PASS |
| **1D-CNN (`3_aec_cnn_reclassify.py`, 이 문서)** | **+0.052** | **+0.038** | **PASS / PASS** |
