# Stage-2 1D-CNN 개선안 비교 (`code/4_*.py` ~ `code/10_*.py`)
...
6장)를 낸 뒤, 이 표본 크기(screen-positive 내부 ~560명)에서 CNN을 더 개선할 수 있는 7가지
방향을 각각 독립된 스크립트로 구현하고, 그중 자동 채점을 통과한 6개 결과(baseline + 6개 변형)를
한 번에 비교하는 스크립트(`100_compare_cnn_variants.py`)를 추가했다. 일곱 번째 변형
(`10_aec_cnn_crossattn.py`)은 안전마진 필터를 통과한 설정이 curve-only(사실상 no-op)로만
수렴해 이 자동 비교 대상에서는 빠지며, 아래 `10_aec_cnn_crossattn.py` 항목에서 별도로 다룬다.

## 1. 설계 방식 — "monkeypatch로 한 가지만 바꾸기"

7개 변형 스크립트는 `3_aec_cnn_reclassify.py`를 **복붙하지 않고 그대로 import한 뒤, 정확히
...
- `3_aec_cnn_reclassify.py`를 고치면 7개 변형에 자동 반영된다 (별도 동기화 불필요).
...
| `9_aec_cnn_fcn.py` | `ResidualCNN`을 시계열 분류 표준 아키텍처인 FCN(128ch/k=8 → 256ch/k=5 → 128ch/k=3, 전 구간 `padding="same"` + GAP)으로 교체 | 학습 절차, 앙상블, 폴드/threshold 로직 | 파라미터 ~150배 증가(~2천 → ~33만), 그리드 없음 | Wang, Yan & Oates, 2017, [Time Series Classification from Scratch with Deep Neural Networks: A Strong Baseline (arXiv:1611.06455)](https://arxiv.org/abs/1611.06455) | Ismail Fawaz et al., 2019, [Deep Learning for Time Series Classification: A Review (Data Min. Knowl. Discov. 33, 917–963)](https://doi.org/10.1007/s10618-019-00619-1) |
| `10_aec_cnn_crossattn.py` | side feature가 curve embedding 전체를 균일하게 스케일/이동시키는 FiLM 대신, side feature로 만든 query가 block3의 32-position 시퀀스에 **cross-attention**해 위치별로 다른 context를 뽑아내도록 함 (`z = pooled + gate·context`, gate는 0-init) | 학습 절차, 앙상블, 폴드/threshold 로직 | 거의 동일 (attention head 파라미터만 소폭 증가) | Vaswani et al., 2017, [Attention Is All You Need (arXiv:1706.03762)](https://arxiv.org/abs/1706.03762) | Lu et al., 2019, [ViLBERT: Pretraining Task-Agnostic Visiolinguistic Representations for Vision-and-Language Tasks (arXiv:1908.02265)](https://arxiv.org/abs/1908.02265) |
...
3. **3차 시도**: 학습 가능한 채널별 residual scale `res_scale`(ReZero/LayerScale 방식,
   ...
   선택이었고, `outputs/100_compare_cnn_variants/spec_delta_comparison.png`(baseline + 7개
   변형 비교)에서 이 설정은 baseline 대비 여전히 크게 뒤처졌다(internal +0.017 vs +0.052,
   external +0.008 vs +0.038) — 현재 자동 비교되는 7개 제안 변형 중에서도 여전히 가장 약한 성능.
...
- 4x2=8개 grid point마다 `3_aec_cnn_reclassify.py`의 sweep 3-설정(`SWEEP_CONFIGS` x
  ...
  복사되어 `100_compare_cnn_variants.py`가 그대로 읽을 수 있게 한다.
...
동일, 같은 비용으로 비교)로 고정한 미검증 추측이었다. 이 고정 설정은 baseline 대비 크게
뒤처졌다(internal +0.008 vs +0.052, external +0.015 vs +0.038) — 당시 비교 대상이던
`5_aec_cnn_skip.py`와 함께 5개 제안 변형 중 가장 약한 성능이었다 (그리드서치 이전 값이라
현재의 `spec_delta_comparison.png`에는 그리드 우승 설정만 실려 있다).
...
### 9. `9_aec_cnn_fcn.py` — 표준 FCN 아키텍처로 교체

Baseline의 `ResidualCNN`은 이 저장소를 위해 직접 설계한 작은 net(conv 3-block, 6/10/10채널,
MaxPool로 128 → 32까지 축소, 파라미터 ~2천개)이다. 이 변형은 그 자리에 시계열/1D-신호 분류에서
가장 널리 쓰이는 "기성" 아키텍처인 FCN(Fully Convolutional Network, Wang, Yan & Oates 2017)을
그대로 끼워 넣는다 — UCR/UEA 시계열 분류 벤치마크와 다수의 생체신호(ECG/EEG/PPG) 분류 논문에서
표준 비교 대상으로 쓰이는 구조다. 논문 표준 채널/커널 폭(128ch/k=8 → 256ch/k=5 → 128ch/k=3)을
그대로 쓰고, 각 블록은 `Conv1d → BatchNorm1d → ReLU`에 `padding="same"`을 적용해 128-포인트
곡선 길이를 세 블록 내내 유지한다(baseline은 대신 `MaxPool1d(2)` 두 번으로 곡선을 줄인다).
마지막 블록의 128채널을 전역평균풀링(GAP)한 뒤 side feature와 concat해 baseline과 동일한
2-layer head(`Linear → ReLU → Dropout → Linear`)에 넣는다. 학습 루프, `N_SEEDS` 앙상블, 조기종료,
augmentation, fold/sweep/threshold 선택, 평가·플롯은 모두 baseline 코드를 monkeypatch로
그대로 재사용한다(1절 참고) — 바뀌는 건 `ResidualCNN` 클래스 하나뿐이다.

미리 짚어둘 캐비어트: FCN의 표준 채널 폭은 파라미터 ~33만개로, baseline의 ResidualCNN(~2천개)
대비 **~150배** 크다. 논문의 채널 폭은 행당 수천 개인 UCR 벤치마크 데이터셋에 맞춰 튜닝된
값으로, screen-positive 내부 ~560명 규모의 이 표본에서 "다른 곳에서 더 널리 쓰인다"가 곧
"이 표본 크기에 더 적합하다"를 뜻하지는 않는다 — 그 실증적 질문에 답하는 것이 이 스크립트의
목적이다. 실제로 이 저장소의 실행 결과(internal spec_delta +0.042, external +0.033, PASS/PASS)는
baseline(+0.052/+0.038)에는 못 미치지만, `4_aec_cnn_pretrain.py`(+0.042/+0.028)와 internal이
동률이고 external은 오히려 더 높다 — 파라미터 수를 150배 늘린 "기성" 아키텍처가 baseline이
설계한 작은 net보다 이 표본에서 뚜렷이 낫다는 근거는 없지만, 못 쓸 정도로 나쁘지도 않다.

### 10. `10_aec_cnn_crossattn.py` — Cross-attention 융합

`8_aec_cnn_film.py`가 이미 보여줬듯, side feature(clinical/stage1 score)가 곡선 임베딩 전체를
균일하게 스케일/이동시키는 것(FiLM)만으로도 baseline의 "맨 마지막에만 concat"보다 나은 조건화가
가능하다. Cross-attention은 이를 한 단계 더 밀고 나간다: 곡선 전체를 하나로 GAP하기 *전에*,
block3 출력(32-position 시퀀스, pooling 없이 유지)에 side feature로 만든 query 하나가
attention해 **위치별로 다른** context를 뽑아낸다 — 즉 환자 프로필이 "곡선 임베딩을 얼마나
키우고 옮길지"뿐 아니라 "곡선의 어느 부분을 읽을지" 자체를 바꿀 수 있다. Transformer의
query/key/value cross-attention(Vaswani et al., 2017)과, 서로 다른 modality를 이런 방식으로
융합하는 멀티모달 모델(예: Lu et al., 2019, ViLBERT)과 같은 메커니즘을 "modality A = 환자별
side feature 벡터 하나, modality B = 32-position 곡선 임베딩 시퀀스"에 적용한 것이다.

**튜닝 이력** (스크립트 상단 주석): 첫 버전은 attention context를 곧바로 head에 넣어(side
feature와 concat), side feature가 있는 모든 설정에서 GAP 임베딩을 완전히 대체했다. 그 결과
side를 쓰는 세 sweep 설정 모두가 `8_aec_cnn_film.py`의 초기(비-identity) 버전이 겪었던 것과
정확히 같은 `sens_delta=-0.0155`에 고정됐고, 이는 비열등성 신뢰구간 상한을 margin의 97.8%
(`ni_ci_upper=0.049` vs `margin=0.05`)까지 밀어붙여 `3_aec_cnn_reclassify.py`의
`SAFE_MARGIN_FRAC=0.8` 안전마진 필터(0.04) 밖으로 밀려났다. side를 쓰는 설정이 전부 "안전하지
않음"으로 걸러지자 `select_best_sweep_config`는 유일하게 남은 curve-only(side feature 전혀
없음) 설정으로 폴백했는데, 이는 사실상 no-op(internal spec_delta +0.001)이고 external
재분류를 한 건도 만들지 못했다(spec_delta=0.0, `verdict=FAIL`) — 즉 첫 버전은 cross-attention이
실제로 도움이 되는지 한 번도 제대로 시험해보지 못한 채 매번 안전마진에 막혀 do-nothing
설정으로 되돌아갔다.

**수정**: `8_aec_cnn_film.py`의 최종 버전과 `5_aec_cnn_skip.py`의 ReZero 시도가 쓴 것과 같은
"초기값에서 항등(identity)" 전략을 적용했다 — attention context로 pooled 임베딩을 대체하는
대신, 학습 가능한 채널별 gate(0으로 초기화)로 섞는다: `z = pooled + gate · context`. 초기
시점엔 `gate=0`이라 `z == pooled`, 즉 학습이 baseline의 이미 검증된 GAP 임베딩에서 출발해서,
side-conditioned context를 실제로 반영할 가치가 있을 때만 서서히 끌어온다.

이 저장소의 실행 결과(`outputs/10_aec_cnn_crossattn/stage2_cnn_sweep_ranking.csv`)에서도
수정 후 여전히 안전마진을 통과하는 설정은 curve-only(use_clinical=False, use_stage1_score=False,
internal spec_delta +0.001) 하나뿐이었다 — side를 쓰는 나머지 세 설정(use_stage1_score=True
조합들)은 모두 동일한 `sens_delta=-0.0155`에 걸려 `ni_ci_upper=0.0489`로 여전히 안전마진
밖이다. 결과적으로 이 변형은 internal spec_delta +0.001, external spec_delta 0.0(재분류 0건,
`verdict=FAIL`)로 baseline은 물론 다른 6개 제안보다도 뚜렷이 약해, `100_compare_cnn_variants.py`의
`VARIANTS` 목록에도 포함하지 않았다 (자동 비교 대상에서 제외한 이유이자, gate 자체가
"do-nothing으로 폴백"만 반복해 cross-attention의 실제 효과를 아직 검증하지 못했다는 뜻이기도
하다).
...
`100_compare_cnn_variants.py`는 이 canonical 위치만 읽으므로 그리드서치 존재 여부와 무관하게
그대로 동작한다.
...
python code/9_aec_cnn_fcn.py

python code/100_compare_cnn_variants.py  # 위에서 실행된 것만 모아 비교

# 10_aec_cnn_crossattn.py는 실행은 가능하지만(outputs/10_aec_cnn_crossattn/) 안전마진을
# 통과하는 설정이 curve-only(no-op)뿐이라 100_compare_cnn_variants.py의 VARIANTS 목록에는
# 없다 — 위 "10. 10_aec_cnn_crossattn.py" 절 참고.
python code/10_aec_cnn_crossattn.py
```

`100_compare_cnn_variants.py`는 **재학습을 전혀 하지 않고**, 이미 저장된
`stage1_vs_stage2_summary.csv`들을 읽어:

- `outputs/100_compare_cnn_variants/comparison_summary.csv` — variant x cohort(internal/external)
  단위로 sens_delta/spec_delta/verdict/ni_verdict/ppv 등을 한 표로 결합
- `outputs/100_compare_cnn_variants/spec_delta_comparison.png` — variant별 internal/external
  spec_delta 막대그래프 (external 막대 위에 PASS/FAIL 라벨 표시)

를 생성한다. 아직 실행하지 않은 변형은 에러 없이 건너뛰고 `[skip] ...` 메시지만 출력하므로,
6개를 전부 돌리지 않고 일부만 먼저 비교해봐도 된다.
...
| fcn (`9_aec_cnn_fcn.py`, 표준 FCN 아키텍처) | +0.042 | PASS | +0.033 | PASS |
| crossattn (`10_aec_cnn_crossattn.py`, 안전마진 통과 설정이 curve-only뿐 — 자동 비교표에는 미포함) | +0.001 | PASS | 0.000 | FAIL |
...
+0.041 vs +0.038), internal에서는 baseline(+0.052)에 못 미친다. `pretrain`과 `fcn`은
internal에서 baseline에 가장 근접하지만(둘 다 +0.042 vs +0.052) external에서는 각각
+0.028, +0.033으로 그 근접함이 그대로 이어지지 않는다. 그리드서치 두 건의 결론은 서로 다르다:
...
`crossattn`은 7개 제안 중 유일하게 external 비열등성 자체는 통과하지만 spec_delta가 0(재분류
0건)이라 판정이 FAIL이다 — 안전마진 필터가 side-conditioned 설정을 전부 걸러내고 매번
curve-only(do-nothing)로 폴백하기 때문에, 이 표본 규모에서 cross-attention이 실제로 유용한지는
아직 검증되지 않은 채로 남아 있다(위 "10. `10_aec_cnn_crossattn.py`" 절 참고).

요컨대 7개 제안 중 어느 것도 "baseline보다 신뢰성 있게 낫다"고 이 시점에서 결론 내릴 근거는
없지만, `bagging`은 그리드서치로 baseline에 가장 가까워진 변형이 됐다 — 아래 4절의 표본 크기
경고는 여전히 유효하다.

## 4. 비교 시 주의할 점

- **공정성의 범위**: 7개 스크립트 모두 baseline과 동일한 Stage-1 모델/th1, AEC 잔차화 회귀,
