...
# Stage-1 TP/FP/TN 그룹 간 raw AEC-128 곡선 비교를, patient-wise 정규화 대신
# 코호트 전체(aec_128 전체 환자) 기준 슬라이스별 global z-score로 정규화해 재확인한다
# (aec_curve_comparison_global.py의 load_data와 동일한 정규화). patient-wise 버전은
# code/stage2_aec_group_comparisons.py 참고. stage2_model.py의 AecBranch 입력은 여전히
# patient-normalized curve 그대로이며, 이 스크립트는 정규화 방식에 따라 그룹 간 곡선
# 차이의 결론이 달라지는지 보기 위한 진단용 비교다:
...
OUT_ROOT = PROJECT_ROOT / "outputs" / "0_clinic-only_baseline" / "aec_curve_comparison_global"

CLIN_COLS = stage2.CLIN_COLS  # ["sex_m", "age_std", "height_std", "weight_std"]
CALIPER_MULT = 0.2
SEED = 42

# p/dp/d2p: raw global z-score normalized AEC-128 curve and its 1st/2nd derivative along the
...
CURVE_TYPE_META = {
    "p": {"suffix": "", "label_suffix": "", "ylabel": "Global z-score AEC", "ref_line": 0.0},
...
def load_aec_for_patients_global(xlsx_path: Path, patient_ids: pd.Series) -> pd.DataFrame:
    # stage2.load_aec_for_patients와 merge/정렬 로직은 동일하지만, 정규화만 patient-mean
    # 나눗셈이 아니라 xlsx의 aec_128 시트 전체 환자를 기준으로 한 슬라이스별 z-score로
    # 바꾼 버전 (aec_curve_comparison_global.py:load_data와 동일한 정규화).
    aec = pd.read_excel(xlsx_path, sheet_name="aec_128", engine="openpyxl")
    curves_raw = aec[stage2.AEC_COLS].astype(float).to_numpy()
    slice_mean = curves_raw.mean(axis=0, keepdims=True)
    slice_std = curves_raw.std(axis=0, keepdims=True, ddof=0)
    norm_curves = (curves_raw - slice_mean) / slice_std

    aec_df = pd.DataFrame(norm_curves, columns=stage2.AEC_COLS)
...
def curve_variant(df: pd.DataFrame, curve_type: str) -> pd.DataFrame:
    """df has AEC_COLS holding global z-score normalized AEC (p) plus other columns
...
def run_cohort(cohort: str, stage1_rows_all: pd.DataFrame, stage1_rows_pos: pd.DataFrame,
    ...
    aec_tp_tn = load_aec_for_patients_global(data_xlsx, tp_tn_rows["PatientID"])
    ...
    aec_fp_tn = load_aec_for_patients_global(data_xlsx, fp_tn_rows["PatientID"])
    ...
    aec_pos_global = load_aec_for_patients_global(data_xlsx, stage1_rows_pos["PatientID"])
    df_tp_fp = aec_pos_global.merge(stage1_rows_pos[["PatientID", "group"]], on="PatientID", how="inner")
    assert len(df_tp_fp) == len(aec_pos_global), f"{cohort}: TP/FP merge dropped rows"
    for ct in curve_types:
        summary_rows.append(_run_curve_comparison(
    ...
    aec_df = aec_pos_global.reset_index(drop=True)
