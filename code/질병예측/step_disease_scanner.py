...
def plot_scanner_auc_bar(scanner_df: pd.DataFrame, feat: str, family_name: str, model_list: list[str],
    ...
    if sub.empty:
        return
    scanners = sorted(pd.unique(sub["scanner"]))
    x = np.arange(len(scanners))
    width = 0.8 / len(model_list)

    fig, axes = plt.subplots(1, 2, figsize=(max(9, 1.6 * len(scanners) + 3) * 2, 6.5))
    for ax, cohort in zip(axes, ["internal", "external"]):
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(scanners, fontsize=14, rotation=30, ha="right")
        ax.set_ylim(0.5, 1.0)
        ax.set_title(f"{feat} ({cohort})", fontsize=18, fontweight="bold", color="#161616")
        ax.set_ylabel("AUC", fontsize=18)
        ax.tick_params(axis="y", labelsize=16)
        ax.grid(alpha=0.3, axis="y")

    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="lower center", ncol=len(model_list), bbox_to_anchor=(0.5, -0.12), fontsize=14,
               frameon=False)
    fig.tight_layout()
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
