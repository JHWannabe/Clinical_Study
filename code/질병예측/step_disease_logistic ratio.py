...
#        Family B: clinic4 vs clinic4+VAT/SAT ratio vs clinic4+VAT/SAT ratio+AEC
#      mean_mAs는 AEC-128 128포인트 곡선(hip-to-liver 구간 관전류)의 환자별 평균(사용자 확인: 기존 mAs
#      메타데이터 컬럼은 분포가 이봉형이라 정의가 불명확해 제외). VAT/SAT는 VAT(내장지방)_SUM과
#      SAT(피하지방)_SUM 두 변수를 각각 넣는 대신 VAT_SUM/SAT_SUM 비율 하나로 통일(사용자 확인 2026-08-24).
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "step_disease_logistic_ratio"
...
MODEL_ORDER = ["clinic4", "clinic4_meanmAs", "clinic4_meanmAs_aec", "clinic4_vatsatratio", "clinic4_vatsatratio_aec"]
MODEL_EXTRA_COLS = {
    "clinic4": [],
    "clinic4_meanmAs": [MEAN_MAS_COL],
    "clinic4_meanmAs_aec": [MEAN_MAS_COL],
    "clinic4_vatsatratio": [VAT_SAT_RATIO_COL],
    "clinic4_vatsatratio_aec": [VAT_SAT_RATIO_COL],
}
MODEL_USES_AEC = {"clinic4": False, "clinic4_meanmAs": False, "clinic4_meanmAs_aec": True,
                   "clinic4_vatsatratio": False, "clinic4_vatsatratio_aec": True}
# DeLong로 비교할 (baseline, 확장모델) 쌍 4개: mean_mAs 단독효과, mean_mAs 위에 AEC 추가효과,
# VAT/SAT ratio 단독효과, VAT/SAT ratio 위에 AEC 추가효과
DELONG_PAIRS = [
    ("clinic4", "clinic4_meanmAs"),
    ("clinic4_meanmAs", "clinic4_meanmAs_aec"),
    ("clinic4", "clinic4_vatsatratio"),
    ("clinic4_vatsatratio", "clinic4_vatsatratio_aec"),
]
MODEL_LABELS = {
    "clinic4": "clinic4",
    "clinic4_meanmAs": "clinic4 + mean mAs",
    "clinic4_meanmAs_aec": "clinic4 + mean mAs + AEC",
    "clinic4_vatsatratio": "clinic4 + VAT/SAT ratio",
    "clinic4_vatsatratio_aec": "clinic4 + VAT/SAT ratio + AEC",
}
# 그래프를 나눌 두 계열: A) clinic4 -> +mean mAs -> +mean mAs+AEC, B) clinic4 -> +VAT/SAT ratio -> +VAT/SAT ratio+AEC
...
FAMILIES = {
    ...
    "vatsatratio": ["clinic4", "clinic4_vatsatratio", "clinic4_vatsatratio_aec"],
}
...
# AEC를 포함하는 모델(clinic4_meanmAs_aec/clinic4_vatsatratio_aec)의 internal OOF 확률. PCA(고유함수 추정)를
...
def plot_roc(feat: str, curves: dict[str, dict[str, np.ndarray]], stats_by_model: dict[str, dict[str, dict]],
    ...
    colors = {"clinic4": "#898781", "clinic4_meanmAs": "#2a78d6", "clinic4_meanmAs_aec": "#1baf7a",
              "clinic4_vatsatratio": "#a35ad1", "clinic4_vatsatratio_aec": "#e2622e"}
...
def main() -> None:
    ...
    for meta in (meta_int, meta_ext):
        vat = pd.to_numeric(meta[VAT_COL], errors="coerce")
        sat = pd.to_numeric(meta[SAT_COL], errors="coerce")
        meta[VAT_SAT_RATIO_COL] = (vat / sat).replace([np.inf, -np.inf], np.nan)

    required_cols = CLINICAL_BASE_COLS + [VAT_SAT_RATIO_COL, MEAN_MAS_COL]
