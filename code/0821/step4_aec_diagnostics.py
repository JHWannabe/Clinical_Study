...
# FPCA n_components는 internal 코호트 AEC 곡선의 scree curve에서 elbow(Kneedle 방식: 축을 0~1로 정규화한 뒤
# 첫점-끝점을 잇는 직선까지 수직거리가 최대인 지점)로 매 run()마다 결정한다(사용자 확인: "R제곱값 말고
# 누적분산비율로 확인해" 이후 "elbow로 교체해서 재확인"). n_components 자체의 표/시각화는
# step1_aec_fpca.py의 몫이라 여기서는 모델 구성에 필요한 값만 계산한다
...
# n_components로 선택한다. (n_components 자체의 표/시각화는 step1_aec_fpca.py의 몫이라 여기서는 모델
# 구성에 필요한 값만 계산). 다운스트림 예측성능(R^2)으로 n을 고르면 그 성능 자체가 선택 기준에 쓰인 데이터로
