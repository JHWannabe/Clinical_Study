...
# clinic4_logistic_regression.py의 clinic4(성별/나이/신장/체중) baseline에 AEC/NAMA 128구간 곡선(liver->pubis,
# aec_128/NAMA_128 시트)을 추가 input으로 넣은 확장 모델. AEC는 FPCA(n=3), NAMA는 FPCA(n=2)로 넣는다(둘 다
# Kneedle/elbow로 재확인한 CURVE_FPCA_N_ELBOW 값). clinic4_aec_vat_logistic.py와 동일 구조에서 VAT curve만
# NAMA(정상감쇠 근육, normal attenuation muscle area)로 바꾼 버전. PCA/scaler는 gangnam 전체에서 한 번만
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "clinic4" / "aec_nama"
N_SLICES = 128
CURVE_SHEETS = {"AEC": ("aec_128", "aec"), "NAMA": ("NAMA_128", "NAMA")}
AEC_FPCA_N = 3  # CURVE_FPCA_N_ELBOW["AEC"](Kneedle/elbow로 재확인)
NAMA_FPCA_N = 2  # CURVE_FPCA_N_ELBOW["NAMA"](Kneedle/elbow로 재확인)
CLINICAL_BASE_COLS = ["PatientAge", "Height", "Weight"]
EXTRA_COLS = ([f"aec_fpca_pc{i}" for i in range(1, AEC_FPCA_N + 1)]
              + [f"nama_fpca_pc{i}" for i in range(1, NAMA_FPCA_N + 1)])
...
# baseline의 load_data(clinic4 결측 제외)에 AEC/NAMA 128구간 곡선을 PatientID로 병합(곡선 없는 환자는 자동 제외)
def load_data_with_curves(path: Path) -> pd.DataFrame:
    ...
    if len(df) < n0:
        print(f"[{path.stem}] AEC/NAMA 곡선 없는 환자 제외: {n0 - len(df)}/{n0}명")
    return df


# AEC/NAMA 128구간 곡선(전처리 없이 원본 그대로)에서 두 곡선의 FPCA 성분을 뽑아 df에 새 컬럼으로 붙인다.
...
def add_curve_features(df: pd.DataFrame, aec_pca: PCA | None = None,
                        nama_pca: PCA | None = None) -> tuple[pd.DataFrame, PCA, PCA]:
    df = df.copy()
    ...
    nama_curve = df[curve_cols(CURVE_SHEETS["NAMA"][1])].to_numpy(dtype=float)
    if nama_pca is None:
        nama_pca = PCA(n_components=NAMA_FPCA_N, random_state=SEED).fit(nama_curve)
    for i, score in enumerate(nama_pca.transform(nama_curve).T, 1):
        df[f"nama_fpca_pc{i}"] = score
    return df, aec_pca, nama_pca
...
def main() -> None:
    ...
    df, aec_pca, nama_pca = add_curve_features(load_data_with_curves(DATA_XLSX))
    x, scaler = clinical_matrix(df)

    externals: dict[str, tuple[np.ndarray, pd.DataFrame]] = {}
    for cohort, path in EXTERNAL_COHORTS.items():
        df_ext, _, _ = add_curve_features(load_data_with_curves(path), aec_pca=aec_pca, nama_pca=nama_pca)
