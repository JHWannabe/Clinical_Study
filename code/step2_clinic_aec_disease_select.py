...
# (사용자 확인: "linear는 제외해") clinic4 vs AEC 형태 feature(SD/Skewness/상하위50%비율/FPCA/전체결합)
# 4개 모델을 전부 logistic regression + internal OOF AUC 기준으로 비교한다.
# FPCA n_components는 internal 코호트 AEC 곡선의 scree curve에서 elbow(Kneedle 방식: 축을 0~1로 정규화한
# 뒤 첫점-끝점을 잇는 직선까지 수직거리가 최대인 지점)로 결정한다(사용자 확인: "R제곱값 말고 누적분산비율로
...
# age/height/weight 행렬 구성 + 표준화 + (include_sex시) sex 열 + (있으면) AEC 형태 feature 결합.
# AEC 파생 feature(SD/Skew/상하위비율/FPCA score)는 원 단위가 서로 달라 clinic feature와 함께 L2
# 정규화 로지스틱에 넣으면 스케일이 큰 feature가 결과를 좌우한다 - step3_clinic_aec_disease_logistic.py의
# clinical_matrix()와 동일하게 aec_extra 전용 StandardScaler로 별도 표준화한다(step2/step3 결과 불일치 수정,
# 사용자 확인 2026-08-14: CKD internal에서 shape_all/FPCA 순위가 스케일링 유무로 뒤집히는 것을 발견)
...
# 이후 "elbow로 교체해서 재확인"), FPCA/PCA 표준 scree test 관행대로 분산 감소 패턴만으로 n을 정한다
...
# FPCA score가 들어간 모델(clinic4_aec_fpca/clinic4_aec_shape_all)의 internal OOF 확률을 산출.
...
def _fpca_oof_proba_predict(meta: pd.DataFrame, aec_raw: np.ndarray, shape_extra_full: np.ndarray | None,
    ...
    for train_idx, test_idx in cv.split(aec_masked, y):
        ...
        x_test, _, _ = clinical_matrix(meta_masked.iloc[test_idx], extra_test, scaler=scaler,
                                        aec_scaler=aec_scaler, include_sex=include_sex)
...
def plot_auc_comparison_table(summary: pd.DataFrame, model_order: list[str], features_dict: dict[str, str],
    ...
    labels = {
        "clinic4": "clinic4",
        "clinic4_aec_shape_no_fpca": "+AEC (SD+Skew+상하위비율)",
        "clinic4_aec_fpca": f"+AEC FPCA(PC1-{fpca_n})",
        "clinic4_aec_shape_all": "+AEC (SD+Skew+상하위비율+FPCA)",
    }
...
# clinic4(include_sex=False면 clinic3) baseline과, +AEC 형태 feature(SD/Skewness/상하위50%비율/전체결합)를
# 추가한 4개 모델을 HTN/DM/CKD 각각에 대해 internal(OOF)/external(frozen) logistic AUC로 비교
def run(meta_int: pd.DataFrame, meta_ext: pd.DataFrame, output_dir: Path, include_sex: bool) -> None:
    ...
    model_order = ["clinic4", "clinic4_aec_shape_no_fpca", "clinic4_aec_fpca", "clinic4_aec_shape_all"]
    models = {
