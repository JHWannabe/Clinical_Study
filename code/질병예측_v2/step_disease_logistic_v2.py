...
# 서브그룹(step_disease_scanner_v2.py로 분리)은 기존 설계를 그대로 유지한다.
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "step_disease_logistic_v2"
...
def run(meta_int: pd.DataFrame, meta_ext: pd.DataFrame, output_dir: Path) -> None:
    ...
    for feat in valid_features:
        ...
        plot_roc(feat, curves, stats_by_model, feat_dir / f"{slug}_roc_curve.png")
