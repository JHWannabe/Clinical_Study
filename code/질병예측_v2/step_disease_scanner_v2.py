...
# code/질병예측/step4_clinic_aec_disease_scanner.py를 베이스로, step_disease_logistic_v2.py가 저장한
# predictions.csv(5개 모델: clinic4/clinic4_meanmAs/clinic4_meanmAs_aec/clinic4_vatsat/clinic4_vatsat_aec,
# 스캐너 제한 해제 코호트)를 재학습 없이 스캐너(Manufacturer)별로 재슬라이싱한다. threshold는 predictions.csv에
# 이미 고정값으로 들어있어 그대로 재사용(internal OOF에서만 탐색 후 동결한 원칙 유지).
...
STEP_DIR = PROJECT_ROOT / "outputs" / "step_disease_logistic_v2"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "step_disease_scanner_v2"
SCANNER_MIN_N = 30
...
def scanner_subgroup_stats(predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    for (feat, model_name, cohort), grp in predictions.groupby(["feature", "model", "cohort"]):
        ...
        y_all, score_all, scanners = grp["y"].to_numpy(), grp["score"].to_numpy(), grp["manufacturer"].to_numpy()
...
def _render_scanner_table_image(table: pd.DataFrame, title: str, out_path: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(28, 1.2 + 0.55 * len(rows)))
    ax.axis("off")
    ...
    ax.set_title(title, fontsize=20, fontweight="bold", color="#161616", pad=10)
    fig.tight_layout()
...
def plot_scanner_subgroup_table_paginated(scanner_df: pd.DataFrame, out_dir: Path,
    ...
    for page_i, chunk in enumerate(chunks, start=1):
        ...
        _render_scanner_table_image(sub, f"스캐너별 서브그룹 Se/Sp/Acc ({page_i}/{len(chunks)}) — {' · '.join(chunk)}",
                                     out_path)
