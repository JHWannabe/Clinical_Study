...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "step2"
...
# sex/age/height/weight 행렬 구성 + 표준화(sex 제외) + (있으면) AEC 형태 feature 결합. scaler 생략 시 새로 학습(내부 코호트용)
def clinical_matrix(meta: pd.DataFrame, aec_extra: np.ndarray | None = None, scaler: StandardScaler | None = None):
    ...
    x = clinic4 if aec_extra is None else np.column_stack([clinic4, aec_extra])
    return x, scaler
...
def plot_r2_comparison(summary: pd.DataFrame, model_order: list[str], out_path: Path) -> None:
    ...
    for ax, cohort in zip(axes, ["internal", "external"]):
        ...
        for i, model_name in enumerate(model_order):
            ...
            ax.bar(x + offset, vals, width, yerr=[err_lo, err_hi], capsize=2,
...
# clinic4 baseline과, clinic4+AEC 형태 feature(SD/Skewness/상하위50%비율/전체결합)를 추가한 4개 모델을
# internal(OOF)/external(frozen)로 비교
def main() -> None:
