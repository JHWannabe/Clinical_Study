...
# step_disease_logistic.py(outputs/01_disease_logistic/logistic)와 output dir을 공유하면 두 스크립트가
# 같은 폴더에 다른 모델 구성을 덮어써 충돌하므로, 이 스크립트 전용 폴더를 따로 둔다
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "0907" / "clinic4_compare"
...
MODEL_ORDER = ["clinic4", "clinic4_fpca3", "clinic4_uplow_ratio"]
MODEL_EXTRA_COLS = {
    "clinic4": [],
    "clinic4_fpca3": [],
    "clinic4_uplow_ratio": [AEC_UPLOW_RATIO_COL],
}
# clinic4_fpca3만 fold별 PCA refit이 필요한 fpca_oof_proba 경로를 탄다(AEC-128 원본에서 직접 파생되는
...
DELONG_PAIRS = [
    ...
    ("clinic4_fpca3", "clinic4_uplow_ratio"),
]
MODEL_LABELS = {
    "clinic4": "clinic4",
    "clinic4_fpca3": "clinic4 + AEC FPCA(3)",
    "clinic4_uplow_ratio": "clinic4 + AEC Upper/Lower ratio",
}
FAMILIES = {
    "aec_compare": ["clinic4", "clinic4_fpca3", "clinic4_uplow_ratio"],
}
