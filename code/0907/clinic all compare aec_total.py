...
# 전용 폴더를 따로 둔다
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "0907" / "clinic9_all_compare_aec_total"
...
# 사용자 확인(2026-09-07): 이 파일은 "all" - kVp/scanner vendor/VAT+SAT/SMI를 전부 결합해 baseline에 추가.
# 전부 실제 크기가 있는 연속형 절대값이라 age/height/weight와 동일하게 StandardScaler 대상에 포함
CLINICAL_BASE_COLS = ["PatientAge", "Height", "Weight", "kVp", VAT_COL, SAT_COL, SMI_COL]
EXCLUDED_PATIENT_IDS: set[int] = set()

# clinic scanner compare.py와 동일한 제조사(모델명)->벤더 매핑. 개별 모델명 단위는 카테고리가 너무 많아
# baseline 공변량으로 쓰기엔 희소하므로 Vendor(Siemens/GE/Philips/Canon) 단위로 묶는다
VENDOR_PREFIXES = {
...
# 실행하지 않고 outputs/0904/clinic_all_compare/logistic_regression_summary.csv의 tuned_C/tuned_penalty
# 결과값을 그대로 옮겨왔다
BEST_LOGREG_PARAMS: dict[str, dict[str, dict]] = {
    "HTN": {
        "clinic9": {"C": 1.0, "penalty": "l1", "solver": "saga"},
        "clinic9_fpca3": {"C": 10.0, "penalty": "l2", "solver": "saga"},
        "clinic9_uplow_ratio": {"C": 10.0, "penalty": "elasticnet", "solver": "saga", "l1_ratio": 0.5},
    },
    "DM": {
        "clinic9": {"C": 0.1, "penalty": "l1", "solver": "saga"},
        "clinic9_fpca3": {"C": 1.0, "penalty": "l1", "solver": "saga"},
        "clinic9_uplow_ratio": {"C": 1.0, "penalty": "l1", "solver": "saga"},
    },
    "CKD": {
        "clinic9": {"C": 10.0, "penalty": "l1", "solver": "saga"},
        "clinic9_fpca3": {"C": 1.0, "penalty": "l1", "solver": "saga"},
        "clinic9_uplow_ratio": {"C": 1.0, "penalty": "l1", "solver": "saga"},
    },
}

# clinic9_fpca3_total/clinic9_uplow_ratio_total은 aec_total 신규 추가(사용자 요청 2026-09-07)라 아직
...
for _feat_params in BEST_LOGREG_PARAMS.values():
    _feat_params["clinic9_fpca3_total"] = dict(_DEFAULT_LOGREG_PARAMS)
    _feat_params["clinic9_uplow_ratio_total"] = dict(_DEFAULT_LOGREG_PARAMS)

FEATURES: dict[str, str] = {"HTN": "htn", "DM": "dm", "CKD": "ckd"}

# clinic4(age/sex/height/weight)에 kVp+scanner_vendor+VAT+SAT+SMI 5개를 더 baseline에 추가한 "all" 결합
# 이라 4+5=9, "clinic9"로 명명(사용자 확인 2026-09-07: kVp+scanner+VAT/SAT+SMI 전부 결합)
MODEL_ORDER = ["clinic9", "clinic9_fpca3", "clinic9_uplow_ratio", "clinic9_fpca3_total", "clinic9_uplow_ratio_total"]
MODEL_EXTRA_COLS = {
    "clinic9": [],
    "clinic9_fpca3": [],
    "clinic9_uplow_ratio": [AEC_UPLOW_RATIO_COL],
    "clinic9_fpca3_total": [],
    "clinic9_uplow_ratio_total": [AEC_UPLOW_RATIO_TOTAL_COL],
}
# clinic9_fpca3만 fold별 PCA refit이 필요한 fpca_oof_proba 경로를 탄다(AEC-128 원본에서 직접 파생되는
# 유일한 모델). uplow_ratio는 환자별로 이미 확정된 값이라 다른 clinic extra_cols와 동일하게 취급
MODEL_USES_AEC = {"clinic9": False, "clinic9_fpca3": True, "clinic9_uplow_ratio": False,
                   "clinic9_fpca3_total": True, "clinic9_uplow_ratio_total": False}
...
MODEL_AEC_SOURCE: dict[str, str | None] = {
    "clinic9": None, "clinic9_fpca3": "128", "clinic9_uplow_ratio": None,
    "clinic9_fpca3_total": "total", "clinic9_uplow_ratio_total": None,
}
DELONG_PAIRS = [
    ("clinic9", "clinic9_fpca3"),
    ("clinic9", "clinic9_uplow_ratio"),
    ("clinic9", "clinic9_fpca3_total"),
    ("clinic9", "clinic9_uplow_ratio_total"),
    ("clinic9_fpca3", "clinic9_fpca3_total"),
    ("clinic9_uplow_ratio", "clinic9_uplow_ratio_total"),
]
MODEL_LABELS = {
    "clinic9": "clinic9",
    "clinic9_fpca3": "clinic9 + AEC(128) FPCA(3)",
    "clinic9_uplow_ratio": "clinic9 + AEC(128) Upper/Lower ratio",
    "clinic9_fpca3_total": "clinic9 + AEC(total) FPCA(3)",
    "clinic9_uplow_ratio_total": "clinic9 + AEC(total) Upper/Lower ratio",
}
...
# clinic9(age/height/weight/kVp/VAT/SAT/SMI+sex+vendor_code) + extra_cols(clinic9_uplow_ratio 모델의
# AEC Upper/Lower 비율처럼 fold와 무관하게 이미 확정된 값) + (aec_extra가 있으면 clinic9_fpca3 모델의
# fold별 FPCA 점수)를 결합. sex/vendor_code는 순서·크기 의미가 없는 명목형이라 스케일링하지 않고 그대로
# 붙이고, 실제 크기가 있는 연속형(age/height/weight/kVp/VAT/SAT/SMI)만 StandardScaler(clinic용/AEC용)로
# 표준화. scaler는 internal에서 fit해 external에 frozen 적용
...
# AEC를 포함하는 모델(clinic9_fpca3)의 internal OOF 확률. PCA(고유함수 추정)를 검증 fold를 제외한 학습
...
def plot_or_forest(feat: str, coef_sheets: dict[str, pd.DataFrame], model_list: list[str], out_path: Path) -> None:
    colors = {"clinic9": "#898781", "clinic9_fpca3": "#1baf7a", "clinic9_uplow_ratio": "#e2622e",
              "clinic9_fpca3_total": "#1baf7a", "clinic9_uplow_ratio_total": "#e2622e"}
...
def run(meta_int: pd.DataFrame, meta_ext: pd.DataFrame, output_dir: Path) -> None:
    ...
    for feat in valid_features:
        ...
        for model_name in MODEL_ORDER:
            ...
            input_cols = (["sex_M", "vendor_code", "age", "height", "weight", "kvp", "vat", "sat", "smi"]
