...
# 전용 폴더를 따로 둔다
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "0907" / "clinic8_all_ratio_compare_aec_total"
...
# 사용자 확인(2026-09-07): 이 파일은 "all ratio" - kVp/scanner vendor/VSR/SMI를 전부 결합해 baseline에
# 추가. kVp/VSR/SMI는 실제 크기가 있는 연속값이라 age/height/weight와 동일하게 StandardScaler 대상
CLINICAL_BASE_COLS = ["PatientAge", "Height", "Weight", "kVp", VATSAT_RATIO_COL, SMI_COL]
EXCLUDED_PATIENT_IDS: set[int] = set()

# clinic scanner compare.py와 동일한 제조사(모델명)->벤더 매핑
VENDOR_PREFIXES = {
...
# 정수 라벨(0/1/2/3)로 인코딩(사용자 확인 2026-09-07: "정수 라벨로 해줘")
VENDOR_CODE_MAP = {"Siemens": 0, "GE": 1, "Philips": 2, "Canon": 3}
...
# 실행하지 않고 outputs/0904/clinic_all_ratio_compare/logistic_regression_summary.csv의 tuned_C/tuned_penalty
# 결과값을 그대로 옮겨왔다
BEST_LOGREG_PARAMS: dict[str, dict[str, dict]] = {
    "HTN": {
        "clinic8": {"C": 1.0, "penalty": "l2", "solver": "saga"},
        "clinic8_fpca3": {"C": 0.1, "penalty": "l2", "solver": "saga"},
        "clinic8_uplow_ratio": {"C": 1.0, "penalty": "l2", "solver": "saga"},
    },
    "DM": {
        "clinic8": {"C": 0.1, "penalty": "elasticnet", "solver": "saga", "l1_ratio": 0.5},
        "clinic8_fpca3": {"C": 10.0, "penalty": "l1", "solver": "saga"},
        "clinic8_uplow_ratio": {"C": 0.1, "penalty": "l2", "solver": "saga"},
    },
    "CKD": {
        "clinic8": {"C": 1.0, "penalty": "l1", "solver": "saga"},
        "clinic8_fpca3": {"C": 100.0, "penalty": "l2", "solver": "saga"},
        "clinic8_uplow_ratio": {"C": 10.0, "penalty": "l2", "solver": "saga"},
    },
}

# clinic8_fpca3_total/clinic8_uplow_ratio_total은 aec_total 신규 추가(사용자 요청 2026-09-07)라 아직
...
for _feat_params in BEST_LOGREG_PARAMS.values():
    _feat_params["clinic8_fpca3_total"] = dict(_DEFAULT_LOGREG_PARAMS)
    _feat_params["clinic8_uplow_ratio_total"] = dict(_DEFAULT_LOGREG_PARAMS)

FEATURES: dict[str, str] = {"HTN": "htn", "DM": "dm", "CKD": "ckd"}

# clinic4(age/sex/height/weight)에 kVp+scanner_vendor+VSR+SMI 4개를 더 baseline에 추가한 "all ratio"
# 결합이라 4+4=8, "clinic8"로 명명(사용자 확인 2026-09-07: kVp+scanner+VAT/SAT+SMI 전부 결합)
MODEL_ORDER = ["clinic8", "clinic8_fpca3", "clinic8_uplow_ratio", "clinic8_fpca3_total", "clinic8_uplow_ratio_total"]
MODEL_EXTRA_COLS = {
    "clinic8": [],
    "clinic8_fpca3": [],
    "clinic8_uplow_ratio": [AEC_UPLOW_RATIO_COL],
    "clinic8_fpca3_total": [],
    "clinic8_uplow_ratio_total": [AEC_UPLOW_RATIO_TOTAL_COL],
}
# clinic8_fpca3만 fold별 PCA refit이 필요한 fpca_oof_proba 경로를 탄다(AEC-128 원본에서 직접 파생되는
# 유일한 모델). uplow_ratio는 환자별로 이미 확정된 값이라 다른 clinic extra_cols와 동일하게 취급
MODEL_USES_AEC = {"clinic8": False, "clinic8_fpca3": True, "clinic8_uplow_ratio": False,
                   "clinic8_fpca3_total": True, "clinic8_uplow_ratio_total": False}
...
MODEL_AEC_SOURCE: dict[str, str | None] = {
    "clinic8": None, "clinic8_fpca3": "128", "clinic8_uplow_ratio": None,
    "clinic8_fpca3_total": "total", "clinic8_uplow_ratio_total": None,
}
DELONG_PAIRS = [
    ("clinic8", "clinic8_fpca3"),
    ("clinic8", "clinic8_uplow_ratio"),
    ("clinic8", "clinic8_fpca3_total"),
    ("clinic8", "clinic8_uplow_ratio_total"),
    ("clinic8_fpca3", "clinic8_fpca3_total"),
    ("clinic8_uplow_ratio", "clinic8_uplow_ratio_total"),
]
MODEL_LABELS = {
    "clinic8": "clinic8",
    "clinic8_fpca3": "clinic8 + AEC(128) FPCA(3)",
    "clinic8_uplow_ratio": "clinic8 + AEC(128) Upper/Lower ratio",
    "clinic8_fpca3_total": "clinic8 + AEC(total) FPCA(3)",
    "clinic8_uplow_ratio_total": "clinic8 + AEC(total) Upper/Lower ratio",
}
...
def load_cohort(xlsx_path: Path) -> pd.DataFrame:
    ...
    # baseline에 scanner 제조사(Vendor)도 추가(사용자 확인 2026-09-07). one-hot 대신 정수 라벨로 인코딩
    vendor = merged["Manufacturer"].astype(str).map(classify_vendor)
    merged["vendor_code"] = vendor.map(VENDOR_CODE_MAP).astype(float)
...
# clinic8(age/height/weight/kVp/vatsat_ratio/SMI+sex+vendor_code) + extra_cols(clinic8_uplow_ratio
# 모델의 AEC Upper/Lower 비율처럼 fold와 무관하게 이미 확정된 값) + (aec_extra가 있으면 clinic8_fpca3
# 모델의 fold별 FPCA 점수)를 결합. sex/vendor_code는 순서·크기 의미가 없는 명목형이라 스케일링하지 않고
# 그대로 붙이고, 실제 크기가 있는 연속형(age/height/weight/kVp/vatsat_ratio/SMI)만 StandardScaler(clinic
# 용/AEC용)로 표준화. scaler는 internal에서 fit해 external에 frozen 적용
def build_matrix(meta: pd.DataFrame, extra_cols: list[str], aec_extra: np.ndarray | None = None,
    ...
    vendor_code = meta["vendor_code"].to_numpy(dtype=float)
    clinic = np.column_stack([sex_m, vendor_code, scaled])
...
# AEC를 포함하는 모델(clinic8_fpca3)의 internal OOF 확률. PCA(고유함수 추정)를 검증 fold를 제외한 학습
...
def plot_or_forest(feat: str, coef_sheets: dict[str, pd.DataFrame], model_list: list[str], out_path: Path) -> None:
    colors = {"clinic8": "#898781", "clinic8_fpca3": "#1baf7a", "clinic8_uplow_ratio": "#e2622e",
              "clinic8_fpca3_total": "#1baf7a", "clinic8_uplow_ratio_total": "#e2622e"}
...
def run(meta_int: pd.DataFrame, meta_ext: pd.DataFrame, output_dir: Path) -> None:
    ...
    for feat in valid_features:
        ...
        for model_name in MODEL_ORDER:
            ...
            input_cols = (["sex_M", "vendor_code", "age", "height", "weight", "kvp", "vatsat_ratio", "smi"]
                          + extra_cols + [f"{fpca_prefix}{i}" for i in range(1, n_extra_aec + 1)])
