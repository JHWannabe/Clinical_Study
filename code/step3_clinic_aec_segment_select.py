...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "step3" / "segment_select"
...
# sex/age/height/weight 행렬 구성 + 표준화(sex 제외) + AEC 구간평균 결합. scaler 생략 시 새로 학습(내부 코호트용)
def clinical_matrix(meta: pd.DataFrame, aec_seg: np.ndarray, scaler: StandardScaler | None = None):
    ...
    x = np.column_stack([clinic4, aec_seg])
    return x, scaler
...
# clinic4 baseline과 전체 N_SEG구간 사용(k=N_SEG) 기준선을 점선으로 표시. cohort_label로 internal/external 구분
def plot_k_trend(pivot: pd.DataFrame, out_path: Path, cohort_label: str = "internal OOF") -> None:
    feat_order = [
        "IMATA_SUM", "LAMA_SUM", "NAMA_SUM", "TAMA_SUM",
        "SAT(피하지방)_SUM", "VAT(내장지방)_SUM", "Total Fat_SUM",
    ]
    feats = [f for f in feat_order if f in pivot.index]
    ncols = 4
    ...
    for ax, feat in zip(axes_flat, feats):
        y = pivot.loc[feat, [f"top{k}" for k in K_VALUES]].to_numpy(dtype=float)
        ax.plot(K_VALUES, y, marker="o", color="#2a78d6", linewidth=1.5, label="선택된 k개 구간")
        ax.axhline(pivot.loc[feat, "clinic4"], color="gray", linestyle="--", linewidth=1, label="clinic4 baseline")
        ax.axhline(pivot.loc[feat, f"top{N_SEG}"], color="#e2622e", linestyle=":", linewidth=1,
                    label=f"전체 {N_SEG}구간 사용")
        ax.set_xticks(K_VALUES)
        ax.set_title(feat, fontsize=10, fontweight="bold")
        ax.set_xlabel("선택 구간 수 (k)")
        ax.set_ylabel(f"R² ({cohort_label})")
        ax.grid(alpha=0.3)
...
# clinic4 baseline과, clinic4+AEC N_SEG구간 중 상위 k개만 선택한 모델들(k=1..N_SEG)을 나란히 학습/평가.
...
def main() -> None:
    ...
    x_int_all, scaler = clinical_matrix(meta_int, aec_seg_int)
    x_ext_all, _ = clinical_matrix(meta_ext, aec_seg_ext, scaler)
    n_clinic = 4

    model_order = ["clinic4"] + [f"top{k}" for k in K_VALUES]

    input_cols = ["sex_M", "age_z", "height_z", "weight_z"] + seg_names
    ...
    plot_k_trend(pivot, OUTPUT_DIR / "clinic4_aec_segment_select_k_trend.png")
    ...
    plot_k_trend(pivot_ext, OUTPUT_DIR / "clinic4_aec_segment_select_k_trend_external.png", cohort_label="external frozen")
