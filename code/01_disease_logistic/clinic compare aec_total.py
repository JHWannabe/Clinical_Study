...
# 다른 0907 스크립트들과 output dir을 공유하면 같은 폴더에 다른 모델 구성을 덮어써 충돌하므로, 이
# 스크립트 전용 폴더를 따로 둔다
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "0907" / "clinic4_compare_aec_total"
...
# 구간을 128등분 후 선형보간)로 리샘플링해야 한다(사용자 확인: "Upper/Lower ratio는 진행할 수 있겠는데
# FPCA는 어렵지?" -> 맞음, 리샘플링으로 해결)
...
# 실행하지 않고 outputs/0907/clinic4_compare/logistic_regression_summary.csv의 tuned_C/tuned_penalty 결과값을
# 그대로 옮겨왔다
BEST_LOGREG_PARAMS: dict[str, dict[str, dict]] = {
    "HTN": {
        "clinic4": {"C": 0.1, "penalty": "l2", "solver": "saga"},
        "clinic4_fpca3": {"C": 0.1, "penalty": "l1", "solver": "saga"},
        "clinic4_uplow_ratio": {"C": 0.1, "penalty": "l1", "solver": "saga"},
    },
    "DM": {
        "clinic4": {"C": 1.0, "penalty": "elasticnet", "solver": "saga", "l1_ratio": 0.5},
        "clinic4_fpca3": {"C": 100.0, "penalty": "elasticnet", "solver": "saga", "l1_ratio": 0.5},
        "clinic4_uplow_ratio": {"C": 0.1, "penalty": "l2", "solver": "saga"},
    },
    "CKD": {
        "clinic4": {"C": 1.0, "penalty": "l2", "solver": "saga"},
        "clinic4_fpca3": {"C": 100.0, "penalty": "l2", "solver": "saga"},
        "clinic4_uplow_ratio": {"C": 100.0, "penalty": "l1", "solver": "saga"},
    },
}
# clinic4_fpca3_total/clinic4_uplow_ratio_total은 aec_total 신규 추가(사용자 요청 2026-09-07)라 아직
# 그리드서치를 거치지 않음 - 기존 모델들이 튜닝 전에 쓰던 것과 동일한 기본값(C=1.0, L2, lbfgs)을 임시로
# 사용. 필요시 추후 동일한 방식(select_logistic_hyperparams)으로 최적화 가능
_DEFAULT_LOGREG_PARAMS = {"C": 1.0, "penalty": "l2", "solver": "lbfgs"}
for _feat_params in BEST_LOGREG_PARAMS.values():
    _feat_params["clinic4_fpca3_total"] = dict(_DEFAULT_LOGREG_PARAMS)
    _feat_params["clinic4_uplow_ratio_total"] = dict(_DEFAULT_LOGREG_PARAMS)

FEATURES: dict[str, str] = {"HTN": "htn", "DM": "dm", "CKD": "ckd"}

MODEL_ORDER = ["clinic4", "clinic4_fpca3", "clinic4_uplow_ratio", "clinic4_fpca3_total", "clinic4_uplow_ratio_total"]
MODEL_EXTRA_COLS = {
    "clinic4": [],
    "clinic4_fpca3": [],
    "clinic4_uplow_ratio": [AEC_UPLOW_RATIO_COL],
    "clinic4_fpca3_total": [],
    "clinic4_uplow_ratio_total": [AEC_UPLOW_RATIO_TOTAL_COL],
}
# clinic4_fpca3/clinic4_fpca3_total만 fold별 PCA refit이 필요한 fpca_oof_proba 경로를 탄다(AEC 원본에서
# 직접 파생되는 유일한 모델들). uplow_ratio류는 환자별로 이미 확정된 값이라 다른 clinic extra_cols와
# 동일하게 취급
MODEL_USES_AEC = {"clinic4": False, "clinic4_fpca3": True, "clinic4_uplow_ratio": False,
                   "clinic4_fpca3_total": True, "clinic4_uplow_ratio_total": False}
...
MODEL_AEC_SOURCE: dict[str, str | None] = {
    "clinic4": None, "clinic4_fpca3": "128", "clinic4_uplow_ratio": None,
    "clinic4_fpca3_total": "total", "clinic4_uplow_ratio_total": None,
}
DELONG_PAIRS = [
    ("clinic4", "clinic4_fpca3"),
    ("clinic4", "clinic4_uplow_ratio"),
    ("clinic4", "clinic4_fpca3_total"),
    ("clinic4", "clinic4_uplow_ratio_total"),
    ("clinic4_fpca3", "clinic4_fpca3_total"),
    ("clinic4_uplow_ratio", "clinic4_uplow_ratio_total"),
]
MODEL_LABELS = {
    "clinic4": "clinic4",
    "clinic4_fpca3": "clinic4 + AEC(128) FPCA(3)",
    "clinic4_uplow_ratio": "clinic4 + AEC(128) Upper/Lower ratio",
    "clinic4_fpca3_total": "clinic4 + AEC(total) FPCA(3)",
    "clinic4_uplow_ratio_total": "clinic4 + AEC(total) Upper/Lower ratio",
}
...
# clinic4(age/height/weight+sex) + extra_cols(clinic4_uplow_ratio 모델의 AEC Upper/Lower 비율처럼 fold와
# 무관하게 이미 확정된 값) + (aec_extra가 있으면 clinic4_fpca3 모델의 fold별 FPCA 점수)를 각각 별도
# StandardScaler(clinic용/AEC용)로 표준화해 결합. scaler는 internal에서 fit해 external에 frozen 적용
...
# AEC를 포함하는 모델(clinic4_fpca3)의 internal OOF 확률. PCA(고유함수 추정)를 검증 fold를 제외한 학습
...
def plot_or_forest(feat: str, coef_sheets: dict[str, pd.DataFrame], model_list: list[str], out_path: Path) -> None:
    colors = {"clinic4": "#898781", "clinic4_fpca3": "#1baf7a", "clinic4_uplow_ratio": "#e2622e",
              "clinic4_fpca3_total": "#1baf7a", "clinic4_uplow_ratio_total": "#e2622e"}
...
def run(meta_int: pd.DataFrame, meta_ext: pd.DataFrame, output_dir: Path) -> None:
    ...
    for feat in valid_features:
        ...
        for model_name in MODEL_ORDER:
            ...
            if uses_aec:
                # fpca3은 aec_128(크롭+128리샘플) 원본을, fpca3_total은 aec_total을 128포인트로 리샘플한
                # 배열을 쓴다(사용자 요청 2026-09-07: "aec_total 시트 데이터로도 성능비교")
                if aec_source == "128":
            ...
            input_cols = (["sex_M", "age", "height", "weight"] + extra_cols
