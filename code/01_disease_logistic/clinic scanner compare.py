...
# clinic compare.py(outputs/0907/clinic4_compare)와 output dir을 공유하면 두 스크립트가 같은 폴더에 다른
# 모델 구성을 덮어써 충돌하므로, 이 스크립트 전용 폴더를 따로 둔다
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "0907" / "clinic6_scanner_compare"
...
# 사용자 확인(2026-09-07): baseline에 kVp(관전압)도 추가. gangnam/sinchon_final_dataset.xlsx는 80~130kVp로
# 값이 섞여 있어(원본.xlsx와 달리 100kVp 단일값이 아님) 실제 크기·순서가 있는 연속형 공변량이라 age/height/
# weight와 동일하게 StandardScaler 대상에 포함. vendor_code는 명목형(순서 없음)이라 sex_M처럼 별도로 다룸
CLINICAL_BASE_COLS = ["PatientAge", "Height", "Weight", "kVp"]
EXCLUDED_PATIENT_IDS: set[int] = set()

# step_disease_scanner.py와 동일한 제조사(모델명)->벤더 매핑. 개별 모델명 단위는 카테고리가 너무 많아
...
# 사용자 확인(2026-09-07): 더미(one-hot) 대신 정수 라벨(0/1/2/3)로 인코딩
...
# 실행하지 않고 outputs/0904/clinic6_scanner_compare/logistic_regression_summary.csv의 tuned_C/tuned_penalty
# 결과값을 그대로 옮겨왔다
BEST_LOGREG_PARAMS: dict[str, dict[str, dict]] = {
    "HTN": {
        "clinic6": {"C": 1.0, "penalty": "l2", "solver": "saga"},
        "clinic6_fpca3": {"C": 0.1, "penalty": "l1", "solver": "saga"},
        "clinic6_uplow_ratio": {"C": 0.1, "penalty": "l1", "solver": "saga"},
    },
    "DM": {
        "clinic6": {"C": 1.0, "penalty": "l1", "solver": "saga"},
        "clinic6_fpca3": {"C": 10.0, "penalty": "l1", "solver": "saga"},
...
# clinic4(age/sex/height/weight)에 kVp와 scanner vendor 2개를 더 baseline에 추가했다는 의미로 "clinic6"
# 로 명명(사용자 확인 2026-09-07)
MODEL_ORDER = ["clinic6", "clinic6_fpca3", "clinic6_uplow_ratio"]
MODEL_EXTRA_COLS = {
    "clinic6": [],
    "clinic6_fpca3": [],
...
DELONG_PAIRS = [
    ...
    ("clinic6_fpca3", "clinic6_uplow_ratio"),
]
MODEL_LABELS = {
    "clinic6": "clinic6",
    "clinic6_fpca3": "clinic6 + AEC FPCA(3)",
    "clinic6_uplow_ratio": "clinic6 + AEC Upper/Lower ratio",
}
FAMILIES = {
    "aec_compare": ["clinic6", "clinic6_fpca3", "clinic6_uplow_ratio"],
}
...
def load_cohort(xlsx_path: Path) -> pd.DataFrame:
    ...
    # 이 파일에서는 clinic4 baseline에 kVp·scanner 제조사(Vendor)를 추가해 clinic6로 확장(사용자 확인
    # 2026-09-07). vendor는 one-hot 대신 정수 라벨(0=Siemens/1=GE/2=Philips/3=Canon)로 인코딩(사용자 확인)
...
# clinic6(age/height/weight/kVp+sex+vendor_code) + extra_cols(clinic6_uplow_ratio 모델의 AEC Upper/Lower
# 비율처럼 fold와 무관하게 이미 확정된 값) + (aec_extra가 있으면 clinic6_fpca3 모델의 fold별 FPCA 점수)를
# 결합. sex/vendor_code는 순서·크기 의미가 없는 명목형이라 스케일링하지 않고 그대로 붙이고, 실제 크기가
# 있는 연속형(age/height/weight/kVp)만 StandardScaler(clinic용/AEC용)로 표준화. scaler는 internal에서
# fit해 external에 frozen 적용
...
def run(meta_int: pd.DataFrame, meta_ext: pd.DataFrame, output_dir: Path) -> None:
    ...
    for feat in valid_features:
        ...
        for model_name in MODEL_ORDER:
            ...
            input_cols = (["sex_M", "vendor_code", "age", "height", "weight", "kvp"] + extra_cols
