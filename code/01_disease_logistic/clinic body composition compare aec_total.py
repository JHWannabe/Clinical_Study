...
# clinic compare.py / clinic scanner compare.py와 output dir을 공유하면 같은 폴더에 다른 모델 구성을
# 덮어써 충돌하므로, 이 스크립트 전용 폴더를 따로 둔다
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "0907" / "clinic7_body_composition_compare_aec_total"
...
# 사용자 확인(2026-09-07): 이 파일은 clinic scanner compare.py의 kVp+scanner vendor 대신 VAT+SAT+SMI(체성분)를
# baseline에 추가. VAT/SAT/SMI는 실제 크기가 있는 연속형 절대값이라 age/height/weight와 동일하게
# StandardScaler 대상에 포함(step_disease_logistic.py의 clinic4_vatsat 모델과 동일 처리)
CLINICAL_BASE_COLS = ["PatientAge", "Height", "Weight", VAT_COL, SAT_COL, SMI_COL]
...
# 실행하지 않고 outputs/0907/clinic7_body_composition_compare/logistic_regression_summary.csv의
# tuned_C/tuned_penalty 결과값을 그대로 옮겨왔다
BEST_LOGREG_PARAMS: dict[str, dict[str, dict]] = {
    "HTN": {
        "clinic7": {"C": 10.0, "penalty": "l1", "solver": "saga"},
        "clinic7_fpca3": {"C": 1.0, "penalty": "l2", "solver": "saga"},
        "clinic7_uplow_ratio": {"C": 1.0, "penalty": "elasticnet", "solver": "saga", "l1_ratio": 0.5},
    },
    "DM": {
        "clinic7": {"C": 1.0, "penalty": "elasticnet", "solver": "saga", "l1_ratio": 0.5},
        "clinic7_fpca3": {"C": 0.1, "penalty": "l2", "solver": "saga"},
        "clinic7_uplow_ratio": {"C": 1.0, "penalty": "l2", "solver": "saga"},
    },
    "CKD": {
        "clinic7": {"C": 100.0, "penalty": "l2", "solver": "saga"},
        "clinic7_fpca3": {"C": 100.0, "penalty": "l2", "solver": "saga"},
        "clinic7_uplow_ratio": {"C": 100.0, "penalty": "l1", "solver": "saga"},
    },
}

# clinic7_fpca3_total/clinic7_uplow_ratio_total은 aec_total 신규 추가(사용자 요청 2026-09-07)라 아직
...
for _feat_params in BEST_LOGREG_PARAMS.values():
    _feat_params["clinic7_fpca3_total"] = dict(_DEFAULT_LOGREG_PARAMS)
    _feat_params["clinic7_uplow_ratio_total"] = dict(_DEFAULT_LOGREG_PARAMS)

FEATURES: dict[str, str] = {"HTN": "htn", "DM": "dm", "CKD": "ckd"}

# clinic4(age/sex/height/weight)에 VAT/SAT/SMI 3개를 더 baseline에 추가했다는 의미로 "clinic7"로 명명
# (사용자 확인 2026-09-07: "body composition 코드에 SMI도 추가" -> 4+VAT+SAT+SMI=7)
MODEL_ORDER = ["clinic7", "clinic7_fpca3", "clinic7_uplow_ratio", "clinic7_fpca3_total", "clinic7_uplow_ratio_total"]
MODEL_EXTRA_COLS = {
    "clinic7": [],
    "clinic7_fpca3": [],
    "clinic7_uplow_ratio": [AEC_UPLOW_RATIO_COL],
    "clinic7_fpca3_total": [],
    "clinic7_uplow_ratio_total": [AEC_UPLOW_RATIO_TOTAL_COL],
}
# clinic7_fpca3만 fold별 PCA refit이 필요한 fpca_oof_proba 경로를 탄다(AEC-128 원본에서 직접 파생되는
# 유일한 모델). uplow_ratio는 환자별로 이미 확정된 값이라 다른 clinic extra_cols와 동일하게 취급
MODEL_USES_AEC = {"clinic7": False, "clinic7_fpca3": True, "clinic7_uplow_ratio": False,
                   "clinic7_fpca3_total": True, "clinic7_uplow_ratio_total": False}
...
MODEL_AEC_SOURCE: dict[str, str | None] = {
    "clinic7": None, "clinic7_fpca3": "128", "clinic7_uplow_ratio": None,
    "clinic7_fpca3_total": "total", "clinic7_uplow_ratio_total": None,
}
DELONG_PAIRS = [
    ("clinic7", "clinic7_fpca3"),
    ("clinic7", "clinic7_uplow_ratio"),
    ("clinic7", "clinic7_fpca3_total"),
    ("clinic7", "clinic7_uplow_ratio_total"),
    ("clinic7_fpca3", "clinic7_fpca3_total"),
    ("clinic7_uplow_ratio", "clinic7_uplow_ratio_total"),
]
MODEL_LABELS = {
    "clinic7": "clinic7",
    "clinic7_fpca3": "clinic7 + AEC(128) FPCA(3)",
    "clinic7_uplow_ratio": "clinic7 + AEC(128) Upper/Lower ratio",
    "clinic7_fpca3_total": "clinic7 + AEC(total) FPCA(3)",
    "clinic7_uplow_ratio_total": "clinic7 + AEC(total) Upper/Lower ratio",
}
...
# clinic7(age/height/weight/VAT/SAT/SMI+sex) + extra_cols(clinic7_uplow_ratio 모델의 AEC Upper/Lower
# 비율처럼 fold와 무관하게 이미 확정된 값) + (aec_extra가 있으면 clinic7_fpca3 모델의 fold별 FPCA 점수)를
# 결합. sex만 0/1 명목형이라 스케일링하지 않고 그대로 붙이고, 실제 크기가 있는 연속형(age/height/weight/
# VAT/SAT/SMI)은 StandardScaler(clinic용/AEC용)로 표준화. scaler는 internal에서 fit해 external에 frozen 적용
...
# AEC를 포함하는 모델(clinic7_fpca3)의 internal OOF 확률. PCA(고유함수 추정)를 검증 fold를 제외한 학습
...
def plot_or_forest(feat: str, coef_sheets: dict[str, pd.DataFrame], model_list: list[str], out_path: Path) -> None:
    colors = {"clinic7": "#898781", "clinic7_fpca3": "#1baf7a", "clinic7_uplow_ratio": "#e2622e",
              "clinic7_fpca3_total": "#1baf7a", "clinic7_uplow_ratio_total": "#e2622e"}
...
def run(meta_int: pd.DataFrame, meta_ext: pd.DataFrame, output_dir: Path) -> None:
    ...
    for feat in valid_features:
        ...
        for model_name in MODEL_ORDER:
            ...
            input_cols = (["sex_M", "age", "height", "weight", "vat", "sat", "smi"] + extra_cols
