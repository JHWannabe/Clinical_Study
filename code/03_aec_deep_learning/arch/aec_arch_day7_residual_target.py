...
# Day7 — Residual-target learning. clinic4 logistic(fold별로 고정 학습, gradient 차단)의 logit을 그대로
# 최종 logit에 더하고, curve 네트워크는 그 잔차(clinic4가 못 맞추는 부분)만 예측하도록 강제한다.
# 지금까지의 concat/gated/attnpool/crossattn은 curve encoder와 clinic4 encoder를 "같이" 학습해 목적함수가
# 곡선이 clinic4를 보완하는지를 직접 타겟하지 않았음 — 이 구조는 그것을 명시적으로 강제하는 유일한 arm.
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "03_aec_deep_learning" / "arch" / "day7_residual_target"
FUSION_NAME = "residual_target"
CHANNELS = BACKBONE["channels"]
KERNEL_SIZES = BACKBONE["kernel_sizes"]
SEARCH_SPACE = {"embed_dim": [8, 16], "residual_hidden": [0, 16]}
N_REFINE_TOP_K = 3
EPS = 1e-6


class ResidualCurveModel(nn.Module):
    def __init__(self, embed_dim: int, residual_hidden: int):
        super().__init__()
        self.curve_encoder = CurveEncoderGAP(CHANNELS, KERNEL_SIZES, embed_dim)
        if residual_hidden > 0:
            self.residual_head = nn.Sequential(nn.Linear(embed_dim, residual_hidden), nn.ReLU(), nn.Linear(residual_hidden, 1))
        else:
            self.residual_head = nn.Linear(embed_dim, 1)

    def forward(self, curve: torch.Tensor) -> torch.Tensor:
        return self.residual_head(self.curve_encoder(curve))


def fit_clinic4_logit(clinic_train: np.ndarray, y_train: np.ndarray) -> LogisticRegression:
    return LogisticRegression(max_iter=2000).fit(clinic_train, y_train)


def train_residual(curve: np.ndarray, clinic4_logit: np.ndarray, y_raw: np.ndarray, config: dict, seed: int,
    ...
    model = ResidualCurveModel(config["embed_dim"], config["residual_hidden"]).to(DEVICE)
    ...
    base_logit_t = torch.tensor(clinic4_logit[tr_idx].reshape(-1, 1), dtype=torch.float32, device=DEVICE)
    ...
    base_logit_val_t = torch.tensor(clinic4_logit[val_idx].reshape(-1, 1), dtype=torch.float32, device=DEVICE)
    y_val = y_raw[val_idx]
    ...
    for epoch in range(EPOCHS):
        ...
        for start in range(0, n, batch_size):
            ...
            opt.zero_grad()
            final_logit = base_logit_t[idx] + model(batch_curve)
            loss = bce(final_logit, y_t[idx])
            loss.backward()
            opt.step()
            epoch_losses.append(loss.item())
        model.eval()
        with torch.no_grad():
            val_pred = torch.sigmoid(base_logit_val_t + model(curve_val_t)).cpu().numpy().ravel()
...
def predict_residual(model: nn.Module, curve: np.ndarray, clinic4_logit: np.ndarray) -> np.ndarray:
    with torch.no_grad():
        curve_t = torch.tensor(curve, dtype=torch.float32, device=DEVICE)
        base_logit_t = torch.tensor(clinic4_logit.reshape(-1, 1), dtype=torch.float32, device=DEVICE)
        return torch.sigmoid(base_logit_t + model(curve_t)).cpu().numpy().ravel()
...
def evaluate_config(curve_raw: np.ndarray, clinic: np.ndarray, y: np.ndarray, folds: list,
    ...
    for tr_idx, va_idx in folds:
        clf = fit_clinic4_logit(clinic[tr_idx], y[tr_idx])
        logit_tr = clf.decision_function(clinic[tr_idx])
        logit_va = clf.decision_function(clinic[va_idx])
        seed_preds = [
            predict_residual(train_residual(curve[tr_idx], logit_tr, y[tr_idx], config, SEED + s), curve[va_idx], logit_va)
...
def main() -> None:
    ...
    for feature in FEATURES:
        ...
        for tr_idx, va_idx in folds:
            clf = fit_clinic4_logit(clinic_int[tr_idx], y_int[tr_idx])
        ...
        for i, combo in enumerate(combos):
            auc, _ = evaluate_config(curve_int_raw, clinic_int, y_int, folds, combo, ensemble_size=1)
        ...
        for r in ranked:
            combo = r["combo"]
            auc, oof = evaluate_config(curve_int_raw, clinic_int, y_int, folds, combo, ensemble_size=ENSEMBLE_SIZE)
            delta = auc - clinic4_auc_int
        ...
        clinic4_full = fit_clinic4_logit(clinic_int, y_int)
        logit_int_full = clinic4_full.decision_function(clinic_int)
        logit_ext_full = clinic4_full.decision_function(clinic_ext)
        loss_histories, ext_preds = [], []
        for s in range(ENSEMBLE_SIZE):
            hist: list[dict] = []
            model = train_residual(curve_int_final, logit_int_full, y_int, best["combo"], SEED + s, history=hist)
            loss_histories.append(hist)
            ext_preds.append(predict_residual(model, curve_ext_final, logit_ext_full))
