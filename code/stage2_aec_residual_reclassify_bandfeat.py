...
# summary with an alternative, more localized curve featurization: mean residual
# within contiguous equal-width slice bands (8/16/32), swept alongside PCA in the
# same internal-OOF model-selection sweep. See fit_curve_featurizer() for why the
# raw `aec_cropped` sheet was considered and rejected as a feature source.
#
# Run: python code/stage2_aec_residual_reclassify_bandfeat.py
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage2_aec_residual_reclassify_bandfeat"
...
def main() -> None:
    ...
    for cfg in SWEEP_CONFIGS:
        ...
        key = (cfg["model_type"], feat_kind, feat_param, cfg["use_stage1_score"])
        sweep_rows.append({"model_type": cfg["model_type"], "curve_feat_kind": feat_kind,
                            ...
                            "feat_k": feat_state["k"], "sens_delta": sens_delta,
    ...
    fig.suptitle(f"Stage-2 reclassification of screen-positives "
                 f"(clinical + AEC residual, {cfg['curve_feat'][0]}:{cfg['curve_feat'][1]})",
