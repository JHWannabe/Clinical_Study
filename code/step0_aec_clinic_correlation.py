...
CORRELATION_CSV = OUTPUT_DIR / "aec_clinic_correlations.csv"
...
# sex/age/height/weight를 raw 값(sex는 0/1)으로 구성. Pearson r은 선형변환에 불변이므로 표준화 불필요
def clinic4_raw(meta: pd.DataFrame) -> pd.DataFrame:
...
def write_correlation_rows(rows: list[dict]) -> None:
...
def plot_clinic_aec_correlation_heatmap(corr_df: pd.DataFrame, segment_counts: list[int], out_path: Path) -> None:
    INK_PRIMARY = "#161616"
...
# clinic4(sex/age/height/weight) 각 변수와 AEC-128 구간평균(SEGMENT_COUNTS 구간) 간 단순 상관을
# internal/external 코호트별로 산출, CSV와 히트맵으로 저장
def main() -> None:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    meta_int, meta_ext = load_cohort(INTERNAL_XLSX), load_cohort(EXTERNAL_XLSX)
    clinic_int, clinic_ext = clinic4_raw(meta_int), clinic4_raw(meta_ext)
    ...
    write_correlation_rows(corr_rows)
    ...
    plot_clinic_aec_correlation_heatmap(corr_df, SEGMENT_COUNTS,
