...
"""
Run the final AEC low-SMI analysis from raw Excel files only -- standalone edition.
...
Then run:
    ...
    python code/run_from_raw_standalone.py

What this script does
----------------------
This is the same four-stage pipeline as run_from_raw.py, but every helper
module's code has been copied in below instead of imported, so this single
file has no dependency on the other aec_*.py files in this folder.

Because several of those original files independently defined same-named
globals (OUT_DIR, SEED, main, ...), each module's section below has those
names prefixed with a short per-module tag (COND_, UNIV_, LOCK_, ...) so
they cannot collide now that everything shares one namespace. Names that
were always set to the same value everywhere (DATA_DIR) were left as one
shared global instead of being duplicated.

Stage 1. Search and lock interpretable AEC features (LOCK_main)
    output: outputs/aec_lock_smoothed_deesc_gate/locked_gate_features.csv

Stage 2. Train region-guided CNN branch probabilities (PATTERN_main)
    output: outputs/aec_region_cnn_pattern_gate/direct_vote_probabilities.npz

Stage 3. Convert CNN branch probabilities into patient-level AEC scores (BOOST_main)
    output: outputs/aec_direct_vote_auc_boost/direct_vote_auc_boost_scores.csv

Stage 4. Final phenotype enrichment analysis (FINAL_main)
    output: outputs/aec_final_global_quintile_phenotype/
...
# exact_p() returns NaN when a candidate gate never de-escalates anyone (n=0
# samples), which is expected during screening; np.nanmin/np.nanmax on an
# all-NaN row then warn even though the resulting NaN is handled correctly
# downstream (such candidates are already rejected by the deesc_n>=25 check).
warnings.filterwarnings("ignore", message="All-NaN (slice|axis) encountered")


# ==========================================================================
# CONFIG: all paths and run-time constants, resolved once, right here.
#
# Everything below is derived from this script's own location, so the whole
# pipeline is portable: clone/copy the project anywhere and it still finds
# data/ and outputs/ next to it. Edit these values directly to point at a
# different data/output folder or to dry-run the pipeline.
# ==========================================================================
...
# Upstream scripts needed to (re)build DIRECT_VOTE_SCORE_CSV from raw xlsx,
# used only as a fallback by run_if_needed() if that file is missing.
...
def LOCK_main() -> None:
    ...
    ).to_csv(LOCK_OUT_DIR / "internal_prescreen_feature_pool.csv", index=False)
...
def DVOTE_crossfit_config(
    ...
    for seed in DVOTE_SEEDS:
        ...
        for fold_id, (tr, va) in enumerate(stratified_folds(y, seed)):
            val_logits, ext_logits, info = DVOTE_train_one_fold(
...
def reset_shared_random_state() -> None:
    """
    Reset the shared clinical-score fold generator before each stage.

    Without this, the clinical cross-validation folds could drift from stage
    to stage simply because the random generator was already consumed by an
    earlier stage running in the same process.
    """
    global RNG
...
def stage_action(action):
    """Wrap a stage so it starts from the same seeded state every time."""
    def _wrapped():
...
def main() -> None:
    # Paths and DRY_RUN are resolved once in the CONFIG block near the top of
    # this file. Edit them there to point at a different data/output folder.
    ...
    stages = [
        ("Stage 1/4: internal locked feature search", LOCK_main),
        ("Stage 2/4: region-guided CNN branch probabilities", PATTERN_main),
        ("Stage 3/4: direct-vote AEC score generation", BOOST_main),
        ("Stage 4/4: final global quintile phenotype analysis", FINAL_main),
    ]
