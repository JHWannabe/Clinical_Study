...
# (baseline 1D-CNN) and each of its improvement variants (4-8) into one comparison
# table + chart, so the 5 proposed improvements can be read side by side against
# the baseline instead of digging through 6 separate output folders.
#
...
#   python code/4_aec_cnn_pretrain.py     (self-supervised encoder pretraining)
...
# then:
#   python code/9_compare_cnn_variants.py
#
...
OUTPUT_DIR = OUTPUTS_DIR / "9_compare_cnn_variants"
