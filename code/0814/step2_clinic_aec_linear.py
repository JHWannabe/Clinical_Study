...
# clinic4 baseline과 비교한다. FPCA의 n_components는 internal OOF R^2가 가장 높은 후보를 매 run()마다 선택한다.
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "step2_linear"
...
def plot_fpca_component_search(search_df: pd.DataFrame, best_n: int, out_path: Path) -> None:
    ...
    ax.set_ylabel("Internal OOF R²", fontsize=42)
    ax.set_title("FPCA 컴포넌트 수별 internal R² (clinic4+FPCA)", fontsize=40, fontweight="bold",
                 color="#161616", pad=30)
    ax.grid(alpha=0.3)
    ax.tick_params(axis="both", labelsize=30)
    ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.28), ncol=3, fontsize=30, frameon=False)
...
def run(meta_int: pd.DataFrame, meta_ext: pd.DataFrame, output_dir: Path, include_sex: bool) -> None:
    ...
    best_n, fpca_search_df = select_best_fpca_n(meta_int, aec_int_raw, include_sex, fpca_target_features, cv)
    plot_fpca_component_search(fpca_search_df, best_n, output_dir / "fpca_component_search.png")
