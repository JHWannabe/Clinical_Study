...
OUT_DIR = Path(__file__).resolve().parent / "outputs" / "0629" / "aec_offset_score"
SEED = 20260629
...
def clinical_raw(meta: pd.DataFrame) -> np.ndarray:
    """메타데이터에서 나이/성별(남성=1)/키/몸무게 원시값 행렬을 만듦 (표준화 이전)."""
...
def fit_clinical(x: np.ndarray, y: np.ndarray) -> Pipeline:
    """결측을 중앙값으로 채운 뒤 표준화→로지스틱 회귀(정규화 거의 없음)로 이어지는 임상 모델을 학습."""
...
def apply_clinical(model: Pipeline, x: np.ndarray) -> np.ndarray:
    """학습된 임상 모델을 새 데이터(결측은 그 데이터 자체 중앙값으로 대체)에 적용해 로짓 점수를 산출."""
    x = np.asarray(x, dtype=float)
    med = np.nanmedian(x, axis=0)
    x = np.where(np.isfinite(x), x, med)
...
def curve_features(curve: np.ndarray, prefix: str, n_dct: int = 12) -> tuple[np.ndarray, list[str]]:
    """한 곡선에서 DCT 저주파 계수 12개와, 표준편차/기울기/1·2차 도함수/초중후반 평균/피크 위치 등 15개 모양 통계량을 합쳐 특징과 이름 목록을 반환."""
...
def fixed_aec_features(aec: np.ndarray, sex: np.ndarray, sex_modified: bool = False) -> tuple[np.ndarray, list[str]]:
    """a128/crop 두 곡선의 고정 특징을 합치고, sex_modified가 True면 여성 여부와의 상호작용항(성별별로 다른 효과)까지 추가."""
...
def fit_offset_ridge(x: np.ndarray, y: np.ndarray, offset: np.ndarray, lam: float) -> tuple[float, np.ndarray]:
    """임상 로짓 점수를 오프셋(고정)으로 두고, AEC 특징 x에 대한 절편+L2 정규화 계수를 L-BFGS로 학습 (임상 점수 위에 AEC가 더하는 부분만 추정)."""
    x = np.asarray(x, dtype=float)
...
def choose_lambda_nested(
    ...
    """내부 교차검증으로 여러 lambda 후보에 대해 (임상 오프셋 고정 후) AEC 릿지 로지스틱의 로그손실/Brier를 비교해 최적값을 선택."""
...
def crossfit_offset_score(
    ...
    """외부 폴드마다 내부 교차검증으로 lambda를 다시 고르는 중첩 교차검증(nested CV)으로, train 전체에 대한
    out-of-fold 임상/AEC/결합 점수를 만듦 (폴드별 선택된 lambda 기록도 함께 반환)."""
...
def train_external_offset_score(
    ...
    """train 전체로 최종 lambda를 고르고 임상+AEC 오프셋 모델을 학습해, train과 외부 데이터의 임상/AEC 점수를 모두 계산."""
...
def metric_row(model: str, y: np.ndarray, score: np.ndarray, threshold: float | None = None) -> dict:
    """모델 이름과 점수로부터 AUC/AP/로그손실/Brier를 계산하고, threshold가 주어지면 혼동행렬 지표까지 한 행에 담음."""
    prob = sigmoid(score)
    row = {
        "model": model,
...
def bootstrap_delta(y: np.ndarray, score_a: np.ndarray, score_b: np.ndarray, n_boot: int = 2500) -> dict:
    """두 점수(a vs b)의 AUC/AP/로그손실/Brier 차이를 부트스트랩 재표본추출로 신뢰구간과 함께 추정."""
...
def run_model(train: dict, test: dict, sex_modified: bool) -> dict:
    """고정 저차원 AEC 특징(성별 상호작용 포함 여부에 따라 두 버전)에 대해 중첩CV OOF 점수와 외부 점수를
    모두 구하고, 조건부 LRT·부트스트랩 델타를 계산해 CSV/결과 딕셔너리로 정리."""
...
def main() -> None:
    """
    이 스크립트의 핵심 실행 흐름 (질문: 결과값을 보고 나서 특징을 고르는 게 아니라, "사전에 고정된"
    저차원 AEC 요약 특징만으로도 임상변수 대비 추가 정보가 있는가? — 데이터 탐색적 윈도우/랜드마크
    선택 없이):

    1. train(g1090)/test(sdata)를 로드한다.
    2. run_model을 두 번 실행: (a) 성별 상호작용이 없는 기본 버전(main_offset_aec), (b) 모든
       AEC 특징에 여성 여부를 곱한 상호작용항을 추가한 버전(sex_modified_offset_aec).
       각 실행은: fixed_aec_features로 a128/crop 곡선의 DCT+모양 통계량(27개, 상호작용 있으면 54개)을
       뽑고, crossfit_offset_score로 중첩교차검증 OOF 점수를, train_external_offset_score로
       외부 데이터 점수를 구한다 (둘 다 "임상 로짓을 오프셋으로 고정하고 그 위에 AEC 릿지를
       얹는" 방식이라 임상 신호는 그대로 유지된 채 AEC의 추가 기여만 분리해서 본다).
    3. 각 버전마다 train/외부 성능표(clinical/combined/aec_only)를 CSV로 저장하고, 조건부
       우도비검정(LRT)과 결합-임상 부트스트랩 델타를 계산한다.
    4. 두 버전의 결과를 "탐색적 윈도우/랜드마크 선택 없는 고정 저차원 특징" 방법론 설명과 함께
       JSON으로 저장하고, 외부 지표만 모은 요약표도 CSV로 저장한다.
    """
