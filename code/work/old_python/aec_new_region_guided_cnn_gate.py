...
def main() -> None:
    ...
    # --- 데이터 준비 ---
    # 내부(g1090)/외부(sdata) 데이터셋을 로드하고, 임상 점수 기준 목표 운영점(S80/S85/S90) 임계값과
    # 환자별 임상양성(cpos) 여부를 계산. AEC 곡선은 채널(원곡선/기울기/곡률 등)로 변환해 내부 기준으로 표준화.
    ...
    # --- 1단계: 설정(CONFIGS)별 4-지역 CNN 교차검증 학습 ---
    # 설정마다 NewRegionGuidedCnn을 여러 시드 x 5-fold로 학습시켜 내부 OOF/외부 확률을 얻고,
    # 임상 단독 대비 AUC 향상을 비교한 뒤, 확률/학습 로그를 저장.
    ...
    # --- 2단계: 설정별 임계값 x 패턴마스크 게이트 전수 탐색 ---
    # 각 설정의 4-지역 확률에 대해 threshold_vectors()로 만든 임계값 조합과 내부 상위 패턴 마스크 조합을
    # 모두 평가해 de-escalation 규칙 후보를 수집.
    all_candidates = []
    for cfg_idx, cfg in enumerate(CONFIGS, start=1):
    ...
    # --- 3단계: 전체 설정 통합 랭킹 ---
    # 모든 설정의 후보를 합쳐 internal_pass -> internal_selection_score -> internal_mean_accuracy 순으로
    # 정렬하고, 내부 통과 후보/내부+외부 모두 통과 후보를 각각 별도 CSV로 저장.
    ...
    # --- 4단계: 최종 우승 규칙(internal_locked / internal_external_audit) 상세 산출 ---
    # 각 랭킹에서 1위 규칙을 골라 상세 de-escalation 지표를 재계산하고, 결과 CSV와 그래프로 저장.
    winners = {
