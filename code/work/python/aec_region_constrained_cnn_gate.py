...
OUT_DIR = Path(__file__).resolve().parent / "outputs" / "0701" / "aec_region_constrained_cnn_gate"
...
def set_seed(seed: int) -> None:
    """numpy/torch(및 가능하면 CUDA)의 난수 시드를 고정해 학습 재현성을 확보."""
    np.random.seed(seed)
...
def row_z(x: np.ndarray) -> np.ndarray:
    """환자(행) 단위로 평균 0, 표준편차 1로 표준화해 절대 레벨이 아니라 곡선 형태만 남김."""
...
def d1(x: np.ndarray) -> np.ndarray:
    """곡선의 1차 차분(기울기)을 계산하고 첫 열을 복제해 길이를 원래와 동일하게 유지."""
    v = np.diff(x, axis=1)
    return np.column_stack([v[:, :1], v])


def d2(x: np.ndarray) -> np.ndarray:
    """1차 차분을 다시 차분해 2차 차분(곡률)을 계산하고 길이를 원래와 동일하게 유지."""
    v = np.diff(d1(x), axis=1)
    return np.column_stack([v[:, :1], v])


def make_channels(norm: np.ndarray) -> np.ndarray:
    """정규화 곡선/기울기/곡률 3채널을 각각 환자별로 표준화(row_z)해 CNN 입력용 (N, 3, 128) 텐서를 만듦 — 형태만 보고 절대 레벨은 못 보게 강제."""
...
def standardize_channels_train_apply(xg: np.ndarray, xs: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """훈련(g1090) 채널의 채널별 평균/표준편차로 훈련·외부 데이터를 함께 표준화."""
...
def soft_2of4_prob(branch_logits: torch.Tensor) -> torch.Tensor:
    """4개 영역 지점(branch)의 로짓으로부터, "4개 중 정확히 0개 또는 1개만 투표"할 확률을 빼서 "2개 이상 투표"할 미분가능한 확률을 계산 — 이산적인 2-of-4 규칙을 매끄러운 손실함수로 근사."""
...
class RegionBranch(nn.Module):
    """AEC 곡선의 특정 해부학적 구간 하나만 보고 "저위험(low-SMI 아님)"에 투표하는 소형 1D CNN 지점(branch)."""

    def __init__(self, in_channels: int = 3, hidden: int = 8, dropout: float = 0.25) -> None:
        """2개의 합성곱 레이어로 구성된 소형 CNN 몸통과, 평균+최댓값 풀링 결과를 받는 드롭아웃+선형 분류 head를 초기화."""
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """구간 입력을 합성곱한 뒤 평균+최댓값 풀링해 이 구간 하나만의 투표 로짓을 계산."""
        z = self.net(x)
...
class RegionConstrainedCnn(nn.Module):
    """REGIONS에 정의된 4개 해부학적 구간마다 독립적인 RegionBranch를 두고, "2개 이상 투표하면 저위험"이라는 해석 가능한 게이트 규칙으로 결합하는 전체 모델."""

    def __init__(self, dropout: float = 0.25) -> None:
        """REGIONS에 정의된 구간 목록을 저장하고, 구간마다 하나씩 독립적인 RegionBranch를 만들어 ModuleList로 등록."""
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """각 구간 branch의 로짓과, 그것들을 soft_2of4_prob로 결합한 최종 저위험 게이트 확률을 반환."""
        logits = []
...
def train_loss(
    ...
    """게이트 확률과 각 branch 보조 확률에 대해, 실제 저SMI(위험군)에는 더 큰 가중치(low_weight)를 주는 가중 이진교차엔트로피 손실을 계산해 합침 — 위험군을 놓치는 실수를 더 강하게 벌점."""
...
def clinical_positive_weights(c_g: np.ndarray, thresholds: dict[str, float]) -> np.ndarray:
    """5개 운영점 임계값 중 몇 개를 넘는 임상양성인지에 비례해 훈련 샘플 가중치를 매김 — de-escalation이 실제로 적용될 임상양성군 학습에 더 집중시키기 위함."""
...
def stratified_folds(y: np.ndarray, seed: int) -> list[tuple[np.ndarray, np.ndarray]]:
    """라벨 비율을 유지하며 5-fold 교차검증 분할(훈련/검증 인덱스 쌍)을 만듦."""
...
def predict_model(model: RegionConstrainedCnn, x: np.ndarray, batch_size: int = 512) -> tuple[np.ndarray, np.ndarray]:
    """배치 단위로 모델을 평가모드로 실행해 전체 데이터의 게이트 확률과 4개 branch 확률을 반환."""
    model.eval()
    gates = []
    branches = []
    with torch.no_grad():
...
def train_one_fold(
    ...
    """한 fold에 대해 RegionConstrainedCnn을 조기종료(early stopping)로 학습시키고, 검증셋(OOF)과 외부셋에 대한 게이트/branch 예측 및 학습 로그를 반환."""
    set_seed(seed)
...
def crossfit_config(
    ...
    """여러 시드 x 5-fold로 반복 학습(train_one_fold)해, 시드에 걸쳐 평균낸 내부 OOF/외부 게이트·branch 예측과 전체 학습 로그를 반환 — 단일 학습의 우연성을 줄이기 위한 다중시드 앙상블."""
    oof_gates_all = []
    ext_gates_all = []
    oof_branches_all = []
    ext_branches_all = []
...
def finite_summary(rows: list[dict], dataset: str) -> dict:
    """지정 데이터셋의 여러 운영점 결과 중 가장 나쁜 경우를 뽑아 안전성 제약 판정용 요약통계로 압축."""
...
def evaluate_cutoff(
    ...
    """CNN 게이트 확률이 주어진 cutoff 이상이면 강등하는 규칙을 내부/외부 x 모든 운영점에 적용해 de-escalation 성능 지표 행들을 계산."""
    rows = []
...
def choose_cutoff_internal(config_name: str, gate_g: np.ndarray, g: dict, c_g: np.ndarray, thresholds: dict[str, float]) -> tuple[float, pd.DataFrame]:
    """g1090 내부 OOF 게이트 점수의 여러 분위수 후보 컷오프를 전수 평가해, 안전성 제약을 통과하며 점수가 가장 높은 컷오프를 g1090만 보고 선택 — 외부 데이터는 전혀 사용하지 않음."""
...
def clinical_plus_auc(
    ...
    """저위험 게이트 확률을 위험 방향의 로짓 점수로 뒤집은 뒤, 임상점수와 함께 로지스틱 결합모델을 5-fold OOF로 학습해 내부·외부 결합 예측 점수를 반환."""
...
def make_auc_rows(config_name: str, g: dict, s: dict, clinical_oof: np.ndarray, clinical_ext: np.ndarray, gate_g: np.ndarray, gate_s: np.ndarray) -> pd.DataFrame:
    """임상단독/CNN게이트단독/결합 모델의 내부·외부 AUC와 임상단독 대비 증분을 비교표로 만듦."""
...
def branch_summary(config_name: str, branch_g: np.ndarray, branch_s: np.ndarray, g: dict, s: dict) -> pd.DataFrame:
    """각 구간(branch)이 저SMI군과 비-저SMI군에서 각각 평균적으로 얼마나 "비-저위험"에 투표하는지 비교해, 구간별 투표 방향이 임상적으로 말이 되는지 확인하는 표를 만듦."""
    rows = []
...
def plot_details(detail: pd.DataFrame, branch_df: pd.DataFrame, out_path: Path) -> None:
    """운영점별 특이도이득/민감도손실, 강등군 사건율, 구간별 투표방향을 3패널 그래프로 그려 PNG로 저장."""
...
def main() -> None:
    """
    이 스크립트의 핵심 실행 흐름 (질문: 손수 고른 해부학적 4개 구간(R1~R4, 미세 최적화된 특징 근처의
    넓은 창)만 보는 작은 CNN들이, "4개 중 2개 이상 동의하면 저위험" 이라는 해석 가능한 규칙으로
    de-escalation 게이트를 만들 수 있는가? — 완전한 블랙박스 CNN 대신 구조적으로 제약된 CNN 사용):

    1. g1090/sdata를 로드하고 곡선/기울기/곡률 3채널 텐서(make_channels)를 만들어 훈련 기준 채널별
       표준화, 임상양성 정도에 비례한 샘플가중치까지 계산.
    2. 2가지 설정(CONFIGS: balanced_aux, low_smi_guard)마다, 여러 시드 x 5-fold로
       RegionConstrainedCnn을 학습(crossfit_config) — 각 구간 branch가 저위험 여부에 투표하고,
       미분가능한 2-of-4 게이트 확률로 결합, 위험군에 더 큰 가중치를 준 손실로 학습.
    3. 각 설정에 대해 g1090 내부 OOF 게이트 점수만으로 안전 컷오프를 탐색(choose_cutoff_internal)하고,
       그 컷오프를 내부+외부에 적용한 de-escalation 상세 결과를 계산.
    4. 임상단독/CNN게이트단독/결합 모델의 AUC 비교표와, 구간별 투표 방향 요약표도 함께 계산.
    5. 내부 데이터만으로 두 설정 중 어느 것이 더 나은지 최종 모델을 선택(model_summary)하고, 선택된
       설정의 결과를 3패널 그래프로 시각화.
    6. 모든 결과(AUC, de-escalation 상세, 컷오프 탐색, branch 요약, 학습로그, 모델선택, 최고모델
       상세)를 CSV로, 실행 설정을 JSON으로 저장한 뒤 콘솔에 전체 결과를 출력.
    """
...
if __name__ == "__main__":
    # 데이터 로드 -> 3채널(곡선/기울기/곡률) 텐서 생성 -> 설정별 구간제약 CNN 학습(다중시드 x 5-fold)
    # -> g1090 내부만으로 안전 컷오프 탐색 -> 내부+외부 de-escalation/AUC/구간투표 평가 -> 최적 설정
    # 선택 및 결과 저장(CSV/JSON/그래프) 순으로 실행된다.
    main()
