...
# clinic4_logistic_regression.py의 clinic4(성별/나이/신장/체중) baseline에 AEC/VAT 128구간 곡선(liver->pubis,
# aec_128/VFA_128 시트)을 추가 input으로 넣은 확장 모델. AEC는 FPCA(n=3), VAT는 FPCA(n=2)로 넣는다(둘 다
# Kneedle/elbow로 재확인한 CURVE_FPCA_N_ELBOW 값). AEC Upper/Lower ratio는 넣지 않음 - VIF 체크에서
# aec_uplow_ratio가 aec_fpca_pc2와 VIF>10으로 겹치고(clinic4_aec_logistic_regression.py에서 재확인) AUC도
# ratio를 더한다고 나아지지 않아 뺐다. clinic4_aec_logistic_regression.py와 동일하게 PCA/scaler를 gangnam
# 전체에서 한 번만 fit하고, split/grid search/외부검증/출력은 전부 clinic4_logistic_regression.py의
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "clinic4" / "aec_vat"
N_SLICES = 128
CURVE_SHEETS = {"AEC": ("aec_128", "aec"), "VAT": ("VFA_128", "VFA")}
AEC_FPCA_N = 3  # CURVE_FPCA_N_ELBOW["AEC"](Kneedle/elbow로 재확인)
VAT_FPCA_N = 2  # CURVE_FPCA_N_ELBOW["VAT"](Kneedle/elbow로 재확인)
...
EXTRA_COLS = ([f"aec_fpca_pc{i}" for i in range(1, AEC_FPCA_N + 1)]
              + [f"vat_fpca_pc{i}" for i in range(1, VAT_FPCA_N + 1)])
...
# baseline의 load_data(clinic4 결측 제외)에 AEC/VAT 128구간 곡선을 PatientID로 병합(곡선 없는 환자는 자동 제외)
def load_data_with_curves(path: Path) -> pd.DataFrame:
    ...
    if len(df) < n0:
        print(f"[{path.stem}] AEC/VAT 곡선 없는 환자 제외: {n0 - len(df)}/{n0}명")
    return df


# AEC/VAT 128구간 곡선(전처리 없이 원본 그대로)에서 두 곡선의 FPCA 성분을 뽑아 df에 새 컬럼으로 붙인다.
...
def add_curve_features(df: pd.DataFrame, aec_pca: PCA | None = None,
                        vat_pca: PCA | None = None) -> tuple[pd.DataFrame, PCA, PCA]:
    df = df.copy()
    ...
    vat_curve = df[curve_cols(CURVE_SHEETS["VAT"][1])].to_numpy(dtype=float)
    if vat_pca is None:
        vat_pca = PCA(n_components=VAT_FPCA_N, random_state=SEED).fit(vat_curve)
    for i, score in enumerate(vat_pca.transform(vat_curve).T, 1):
        df[f"vat_fpca_pc{i}"] = score
    return df, aec_pca, vat_pca
...
def main() -> None:
    ...
    df, aec_pca, vat_pca = add_curve_features(load_data_with_curves(DATA_XLSX))
    x, scaler = clinical_matrix(df)

    externals: dict[str, tuple[np.ndarray, pd.DataFrame]] = {}
    for cohort, path in EXTERNAL_COHORTS.items():
        df_ext, _, _ = add_curve_features(load_data_with_curves(path), aec_pca=aec_pca, vat_pca=vat_pca)
