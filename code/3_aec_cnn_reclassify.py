...
#
# Same Stage-1/Stage-2 structure as 1_aec_residual_reclassify.py (see that
...
# small setup changes.
#
# Reused as-is from 1_aec_residual_reclassify.py: cohort/AEC loading,
...
#
# Run: python code/3_aec_cnn_reclassify.py
...
residual = importlib.import_module("1_aec_residual_reclassify")

PROJECT_ROOT = Path(__file__).resolve().parents[1]
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "3_aec_cnn_reclassify"
...
#   - 2_aec_residual_reclassify_bandfeat.py's SELECTION_MARGIN trick (pick
...
def main() -> None:
    ...
    for cfg in SWEEP_CONFIGS:
        ...
        sweep_state[key] = {"th2": th2, "sens_delta": sens_delta, "spec_delta": spec_delta, "ok": ok, "cfg": cfg,
                             "ni_ci_upper": ni_sweep["ci_upper"], "margin": ni_sweep["margin"]}
    ...
    best = select_best_sweep_config(sweep_state)
