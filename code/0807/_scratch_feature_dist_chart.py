...
for i, (col, label, direction, cutoff_m, cutoff_f) in enumerate(FEATURES):
    ...
    ax.set_title(f"{label} ({dir_label})", fontsize=36, fontweight="bold", color=INK_PRIMARY)
    ax.set_xlabel(col, fontsize=25.5, color=INK_SECONDARY)
    ax.set_ylabel("환자 수", fontsize=25.5, color=INK_PRIMARY)
    ax.tick_params(axis="both", labelsize=22.5, colors=INK_SECONDARY)
...
ax_legend = axes[7]
ax_legend.axis("off")
ax_legend.text(0.02, 0.88, "범례", fontsize=39, fontweight="bold", color=INK_PRIMARY,
                transform=ax_legend.transAxes)
ax_legend.add_patch(Rectangle((0.02, 0.74), 0.08, 0.08, color=BLUE_LIGHT, alpha=0.65,
                               transform=ax_legend.transAxes))
ax_legend.text(0.14, 0.76, "Male (분포)", fontsize=31.5, color=INK_PRIMARY, transform=ax_legend.transAxes)
ax_legend.add_patch(Rectangle((0.02, 0.62), 0.08, 0.08, color=RED_LIGHT, alpha=0.75,
                               transform=ax_legend.transAxes))
ax_legend.text(0.14, 0.64, "Female (분포)", fontsize=31.5, color=INK_PRIMARY, transform=ax_legend.transAxes)
ax_legend.plot([0.02, 0.10], [0.51, 0.51], color=CUTOFF_M, linestyle="--", linewidth=2.0,
               transform=ax_legend.transAxes)
ax_legend.text(0.14, 0.49, "Male cutoff", fontsize=31.5, color=INK_PRIMARY, transform=ax_legend.transAxes)
ax_legend.plot([0.02, 0.10], [0.39, 0.39], color=CUTOFF_F, linestyle="--", linewidth=2.0,
               transform=ax_legend.transAxes)
ax_legend.text(0.14, 0.37, "Female cutoff", fontsize=31.5, color=INK_PRIMARY, transform=ax_legend.transAxes)
