...
def _draw_scatter(ax: Axes, y: np.ndarray, pred: np.ndarray, s: dict, title: str, lims: tuple[float, float]) -> None:
    ...
    ax.text(0.05, 0.93, f"R² = {s['r2']:.3f}", ha="left", va="top",
             fontsize=66, fontweight="bold", color=INK_PRIMARY, transform=ax.transAxes)
    ax.text(0.05, 0.85, f"95% CI [{s['r2_ci_lower']:.3f}, {s['r2_ci_upper']:.3f}]\n"
             ...
             ha="left", va="top", fontsize=27, color=INK_MUTED, transform=ax.transAxes)

    ax.set_xlabel("Actual")
    ax.set_ylabel("Predicted")
    ax.set_title(title, fontsize=36, fontweight="bold", color=INK_PRIMARY)
    ax.grid(alpha=0.3)
        ...
        for i in range(mat.shape[0]):
            for j in range(mat.shape[1]):
                ...
                ax.text(j, i, f"{r_val:.2f}", ha="center", va="center",
                        fontsize=27, color="white" if abs(r_val) > 0.5 else INK_PRIMARY)

        ax.set_xticks(range(len(input_cols)))
        ax.set_xticklabels([predictor_labels[c] for c in input_cols])
        ax.set_yticks(range(len(features)))
        ax.set_yticklabels(features)
        ax.set_title(f"{cohort}", fontsize=36, fontweight="bold", color=INK_PRIMARY)

    fig.colorbar(im, ax=axes, fraction=0.03, pad=0.02, label="Pearson r")
    fig.suptitle("Output feature vs clinic4 + AEC mean Pearson r", fontsize=39, fontweight="bold",
                 color=INK_PRIMARY)
...
def plot_feature_aec_mean_only_corr_heatmap(corr_df: pd.DataFrame, out_path: Path) -> None:
    ...
    for row, cohort in enumerate(cohorts):
        ...
        for i in range(mat.shape[0]):
            ...
            ax.text(0, i, f"{r_val:.2f}", ha="center", va="center",
                    fontsize=30, color="white" if abs(r_val) > 0.5 else INK_PRIMARY)
    ...
    fig.suptitle("Output feature vs AEC mean Pearson r", fontsize=39, fontweight="bold",
                 color=INK_PRIMARY)
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def plot_clinic4_vs_aec_mean_corr_heatmap(corr_df: pd.DataFrame, input_cols: list[str], out_path: Path) -> None:
    ...
    fig.colorbar(im, ax=axes, fraction=0.04, pad=0.03, label="Pearson r")
    fig.suptitle("clinic4 vs AEC mean", fontsize=39, fontweight="bold", color=INK_PRIMARY, y=1.04)
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def plot_feature_aec_correlation_heatmap(corr_df: pd.DataFrame, segment_counts: list[int], out_path: Path) -> None:
        ...
        for col, cohort in enumerate(cohorts):
            ...
            if n_seg <= 8:
                for i in range(mat.shape[0]):
                    for j in range(mat.shape[1]):
                        r_val = mat.iat[i, j]
                        ax.text(j, i, f"{r_val:.2f}", ha="center", va="center", fontsize=21,
                                color="white" if abs(r_val) > 0.5 else INK_PRIMARY)
            ax.set_yticks(range(len(features)))
            ax.set_yticklabels(features if col == 0 else [], fontsize=24)
            ax.set_xticks([])
            if col == 0:
                ax.set_ylabel(f"n_seg={n_seg}", fontsize=27, fontweight="bold")
            if row == 0:
                ax.set_title(cohort, fontsize=33, fontweight="bold", color=INK_PRIMARY)
    ...
    fig.suptitle("Output feature vs AEC segment-mean Pearson r (row=n_seg)",
                 fontsize=36, fontweight="bold", color=INK_PRIMARY)
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def plot_segment_trend(pivot_int: pd.DataFrame, pivot_ext: pd.DataFrame, model_order: list[str], out_path: Path) -> None:
    ...
    for ax, feat in zip(axes_flat, feats):
        ...
        ax.set_title(feat, fontsize=30, fontweight="bold")
