...
def _draw_residual_row(axes: np.ndarray, y: np.ndarray, pred: np.ndarray, row_title: str) -> None:
    ...
    ax.set_title(f"{row_title}\nResiduals vs Predicted", fontsize=30, fontweight="bold", color=INK_PRIMARY)
    ax.grid(alpha=0.3)
    ...
    ax.set_title(f"Normal Q-Q (r={r:.3f})", fontsize=30, fontweight="bold", color=INK_PRIMARY)
    ax.grid(alpha=0.3)
    ...
    ax.set_ylabel("Count")
    ax.set_title(f"Residual Histogram (Shapiro p={shapiro_p:.2e})", fontsize=30, fontweight="bold", color=INK_PRIMARY)
    ax.grid(alpha=0.3)
    ...
    ax.set_title("Scale-Location", fontsize=30, fontweight="bold", color=INK_PRIMARY)
    ax.grid(alpha=0.3)
...
def plot_metrics_summary(summary: pd.DataFrame, out_path: Path) -> None:
    ...
    for ax, metric in zip(axes.ravel(), metric_labels):
        ...
        ax.set_title(metric_labels[metric], fontsize=36, fontweight="bold", color="#161616")
...
def plot_feature_clinic4_corr_heatmap(corr_df: pd.DataFrame, input_cols: list[str], out_path: Path) -> None:
    ...
    fig.suptitle("Output feature vs clinic4 Pearson r", fontsize=39, fontweight="bold", color=INK_PRIMARY)
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
