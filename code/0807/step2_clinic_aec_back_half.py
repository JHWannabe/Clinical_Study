...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "step2" / "aec_back_half"
...
# age/height/weight 행렬 구성 + 표준화 + (include_sex시) sex 열 + (있으면) AEC 반구간 평균 결합. scaler 생략 시 새로 학습(내부 코호트용).
...
def clinical_matrix(meta: pd.DataFrame, aec_mean: np.ndarray | None = None, scaler: StandardScaler | None = None,
    ...
    x = clinic if aec_mean is None else np.column_stack([clinic, aec_mean])
    return x, scaler
...
def plot_r2_comparison(summary: pd.DataFrame, model_order: list[str], out_path: Path) -> None:
    ...
    for ax, cohort in zip(axes, ["internal", "external"]):
        ...
        for i, model_name in enumerate(model_order):
            vals = [cast(float, rows.loc[(f, model_name), "r2"]) for f in features]
            ax.bar(x + (i - (n_models - 1) / 2) * width, vals, width,
...
# clinic4(include_sex=False면 clinic3) baseline과, +AEC 앞50%/뒤50% 평균을 추가한 두 모델을 internal(OOF)/external(frozen)로 비교
def run(meta_int: pd.DataFrame, meta_ext: pd.DataFrame, output_dir: Path, include_sex: bool) -> None:
    ...
    full_mean_int = meta_int[AEC_COLS].astype(float).to_numpy().mean(axis=1, keepdims=True)
    full_mean_ext = meta_ext[AEC_COLS].astype(float).to_numpy().mean(axis=1, keepdims=True)

    model_order = ["clinic4", "clinic4_aec_full", "clinic4_aec_front50", "clinic4_aec_back50"]
    models = {
        ...
        "clinic4_aec_full": {"aec_int": full_mean_int, "aec_ext": full_mean_ext},
    }
