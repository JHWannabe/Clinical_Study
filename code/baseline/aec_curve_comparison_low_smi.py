...
AEC 프로파일)를 이용해, 성별 / 나이 / TAMA / BMI / Height / Weight로 나눈 그룹
"각각의 내부"에서 Low-SMI vs Non-low SMI AEC "point curve"(슬라이스별 평균 곡선 +
신뢰구간 리본) 비교 그래프를 그린다 (예: 남성 내 Low-SMI vs Non, 여성 내 Low-SMI
vs Non을 나란히 facet으로 배치).

- 강남(gangnam), 신촌(sinchon) 코호트를 각각 독립적으로 분석하여
  outputs/0_clinic-only_baseline/aec_curve_comparison/{gangnam,sinchon}/ 에
  결과를 저장한다.
...
- 변수별 그룹(예: Male/Female, BMI 4구간 등) 각각에 대해 Low-SMI vs Non-low SMI
  비교를 facet subplot으로 배치하고, 전체(비분할) Low-SMI vs Non-low SMI 비교도
  기준선으로 별도 유지한다 (propensity-matched 버전 포함).
...
TWO_GROUP_COLORS = [COL_A, COL_B]  # Low SMI / Non-low SMI 공통 배색
INK_PRIMARY = "#0b0b0b"
...
FACET_MIN_N = 5  # 그룹 내 Low/Non-low SMI 각각 최소 표본 수 (미만이면 검정 생략)


def facet_lowsmi_by_variable(df, var_col, var_order, var_labels, out_dir, fname, title, summary_rows):
    # 변수(예: 성별, BMI 4구간)로 나눈 그룹 각각을 facet subplot으로 두고, 그 안에서
    # Low-SMI vs Non-low SMI 곡선을 비교한다 (변수 그룹 간 비교가 아니라, 변수 그룹을
    # 고정한 채 SMI 효과만 보기 위함).
    n = len(var_order)
    ...
    for ax, level, level_label in zip(axes_flat, var_order, var_labels):
        sub = df[df[var_col] == level]
        plot_curve_comparison(ax, sub, "LowSMI", SMI_ORDER, SMI_LABELS, TWO_GROUP_COLORS,
                              f"{level_label} (n={len(sub)})")
        counts = sub["LowSMI"].value_counts().reindex(SMI_ORDER).fillna(0)
        if counts.min() >= FACET_MIN_N:
            r = curve_diff_test(sub, "LowSMI", SMI_ORDER, SMI_LABELS)
            summary_rows.append({"figure": fname, "comparison": f"{title} | {level_label}", **r})
            ax.text(0.02, 0.02, curve_diff_note(r), transform=ax.transAxes, fontsize=8,
                    color=INK_MUTED, va="bottom")
        else:
            ax.text(0.02, 0.02, f"표본 수 부족 (Low={int(counts['Low SMI'])}, Non={int(counts['Non-low SMI'])})",
                    transform=ax.transAxes, fontsize=8, color=INK_MUTED, va="bottom")

    for ax in axes_flat[n:]:
        ax.axis("off")
    fig.suptitle(title, fontsize=14, color=INK_PRIMARY)
...
# ------------------------------------------------------- propensity matching --
# 01(단순 cutoff 분할, 전체 환자)은 Low-SMI 여부와 성별/나이/키/몸무게가 서로 얽혀 있을 때
# 그 얽힘을 통제하지 못한다. 01b는 같은 두 그룹을 stage2_dataset.CLIN_COLS와 동일한
...
def run_cohort(cohort):
    ...
    # 01: 전체(비분할) Low-SMI vs Non-low SMI 곡선 비교 -- 이후 변수별 facet 비교의 기준선
    fig, ax = plt.subplots(figsize=(8, 5.5))
    plot_curve_comparison(ax, df, "LowSMI", SMI_ORDER, SMI_LABELS, TWO_GROUP_COLORS,
                          "Low-SMI 임상 cutoff에 따른 AEC 곡선 비교 (전체)")
    r_lowsmi_full = curve_diff_test(df, "LowSMI", SMI_ORDER, SMI_LABELS)
    summary_rows.append({"figure": "01_aec_curve_by_low_smi_overall.png",
                          "comparison": "Low-SMI cutoff (전체)", **r_lowsmi_full})
    ax.text(0.02, 0.02, curve_diff_note(r_lowsmi_full),
            transform=ax.transAxes, fontsize=8, color=INK_MUTED, va="bottom")
    fig.tight_layout()
    savefig(fig, out_dir, "01_aec_curve_by_low_smi_overall.png")

    # 01b: Low-SMI, propensity score matching (성별/나이/키/몸무게 균형 후 재비교)
    ...
    bal_path = os.path.join(out_dir, "01b_aec_curve_by_low_smi_matched_balance.csv")
    ...
    fig, ax = plt.subplots(figsize=(8, 5.5))
    plot_curve_comparison(ax, df_matched, "LowSMI", SMI_ORDER, SMI_LABELS,
                          TWO_GROUP_COLORS, "Low-SMI 임상 cutoff에 따른 AEC 곡선 비교 (propensity-matched, 전체)")
    r_psm = curve_diff_test(df_matched, "LowSMI", SMI_ORDER, SMI_LABELS)
    r_psm["n_matched_pairs"] = len(matched_idx) // 2
    summary_rows.append({"figure": "01b_aec_curve_by_low_smi_matched.png",
                          "comparison": "Low-SMI cutoff (propensity-matched, 전체)", **r_psm})
    ax.text(0.02, 0.02, f"{curve_diff_note(r_psm)}\n(matched pairs={r_psm['n_matched_pairs']})",
            transform=ax.transAxes, fontsize=8, color=INK_MUTED, va="bottom")
    fig.tight_layout()
    savefig(fig, out_dir, "01b_aec_curve_by_low_smi_matched.png")

    # 01c: Low-SMI(비매칭), 전체 곡선 permutation test의 peak_slice 주변 구간 확대 비교
    win_cols, win_lo, win_hi = peak_window_cols(r_lowsmi_full["peak_slice"])
    fig, ax = plt.subplots(figsize=(8, 5.5))
    plot_curve_comparison(ax, df, "LowSMI", SMI_ORDER, SMI_LABELS, TWO_GROUP_COLORS,
                          f"Low-SMI AEC 곡선 비교(전체) - peak 구간 확대 (slice {win_lo}-{win_hi})",
                          cols=win_cols)
    r_win = curve_diff_test(df, "LowSMI", SMI_ORDER, SMI_LABELS, cols=win_cols)
    summary_rows.append({"figure": "01c_aec_curve_by_low_smi_peak_window.png",
                          "comparison": f"Low-SMI cutoff (전체), peak 구간 (slice {win_lo}-{win_hi})", **r_win})
    ax.text(0.02, 0.02,
            f"{curve_diff_note(r_win)}\n(전체 곡선 peak={r_lowsmi_full['peak_slice']} 기준 ±{PEAK_WINDOW_HALFWIDTH}슬라이스)",
            transform=ax.transAxes, fontsize=8, color=INK_MUTED, va="bottom")
    fig.tight_layout()
    savefig(fig, out_dir, "01c_aec_curve_by_low_smi_peak_window.png")

    # 01d: Low-SMI(propensity-matched), peak_slice 주변 구간 확대 비교
    win_cols_m, win_lo_m, win_hi_m = peak_window_cols(r_psm["peak_slice"])
    fig, ax = plt.subplots(figsize=(8, 5.5))
    plot_curve_comparison(ax, df_matched, "LowSMI", SMI_ORDER, SMI_LABELS, TWO_GROUP_COLORS,
                          f"Low-SMI AEC 곡선 비교(propensity-matched, 전체) - peak 구간 확대 (slice {win_lo_m}-{win_hi_m})",
                          cols=win_cols_m)
    r_win_m = curve_diff_test(df_matched, "LowSMI", SMI_ORDER, SMI_LABELS, cols=win_cols_m)
    r_win_m["n_matched_pairs"] = len(matched_idx) // 2
    summary_rows.append({"figure": "01d_aec_curve_by_low_smi_matched_peak_window.png",
                          "comparison": f"Low-SMI cutoff (propensity-matched, 전체), peak 구간 (slice {win_lo_m}-{win_hi_m})",
                          **r_win_m})
    ax.text(0.02, 0.02,
            ...
            transform=ax.transAxes, fontsize=8, color=INK_MUTED, va="bottom")
    fig.tight_layout()
    savefig(fig, out_dir, "01d_aec_curve_by_low_smi_matched_peak_window.png")

    # 02-08: 변수별로 나눈 그룹 각각의 내부에서 Low-SMI vs Non-low SMI 곡선 비교 (facet)
    ...
    facet_specs = [
        (df, "PatientSex", ["M", "F"], ["Male", "Female"],
         "02_aec_curve_lowsmi_by_sex.png", "성별 그룹 내 Low-SMI vs Non-low SMI AEC 곡선"),
        (df, "AgeGroup2", ["Low", "High"], ["Age ≤ median", "Age > median"],
         "03_aec_curve_lowsmi_by_age.png", "나이 그룹(median 분할) 내 Low-SMI vs Non-low SMI AEC 곡선"),
        (df, "TAMAGroup2", ["Low", "High"], ["TAMA ≤ 성별 median", "TAMA > 성별 median"],
         "04_aec_curve_lowsmi_by_tama.png", "TAMA 그룹(성별 median 분할) 내 Low-SMI vs Non-low SMI AEC 곡선"),
        (df, "HeightGroup2", ["Low", "High"], ["Height ≤ median", "Height > median"],
         "05_aec_curve_lowsmi_by_height.png", "신장 그룹(median 분할) 내 Low-SMI vs Non-low SMI AEC 곡선"),
        (df, "WeightGroup2", ["Low", "High"], ["Weight ≤ median", "Weight > median"],
         "06_aec_curve_lowsmi_by_weight.png", "체중 그룹(median 분할) 내 Low-SMI vs Non-low SMI AEC 곡선"),
        (df, "BMIGroup2", ["Low", "High"], ["BMI ≤ 성별 median", "BMI > 성별 median"],
         "07_aec_curve_lowsmi_by_bmi.png", "BMI 그룹(성별 median 분할) 내 Low-SMI vs Non-low SMI AEC 곡선"),
        (bmi4_df, "BMIGroup4", bmi4_order, bmi4_order,
         "08_aec_curve_lowsmi_by_bmi4.png", "BMI 4구간 내 Low-SMI vs Non-low SMI AEC 곡선"),
    ]
    for gdf, var_col, order, labels, fname, title_kr in facet_specs:
        facet_lowsmi_by_variable(gdf, var_col, order, labels, out_dir, fname, title_kr, summary_rows)

    # 09: 스캐너 제조사(Vendor, n>=30) 그룹 내 Low-SMI vs Non-low SMI 곡선
    ...
    if len(vendor_order) >= 2:
        ...
        facet_lowsmi_by_variable(vendor_df, "Vendor", vendor_order, vendor_order, out_dir,
                                  "09_aec_curve_lowsmi_by_vendor.png",
                                  "스캐너 제조사(Vendor) 그룹 내 Low-SMI vs Non-low SMI AEC 곡선", summary_rows)
    else:
        print(f"skip vendor facet: only {len(vendor_order)} vendor group(s) with n>=30 ({vendor_order})")
